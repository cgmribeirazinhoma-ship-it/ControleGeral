...
const TINFO = {
  padrao: { label: "Anexo II", icon: "📄", cor: "#3b82f6" },
  eng: { label: "NF Engenharia", icon: "🏗️", cor: "#f97316" },
  tdf: { label: "TFD", icon: "🏥", cor: "#10b981" },
  passagem: { label: "Restituição Passagem", icon: "✈️", cor: "#8b5cf6" },
...
const FOOTER_TXT = "RUA IMPERATRIZ II, Nº 800, CENTRO - RIBEIRÃOZINHO DO MARANHÃO-MA  |  CEP: 65.928-000";
const MUN = {
  gold: "#EFD103",
  goldDk: "#b89d00",
  goldXdk: "#7a6500",
...
const T = {
  appBg: "#f0f4fb", cardBg: "#ffffff", border: "#b8cce8", textMain: "#0f1e36",
  appBgDark: "#0a0d14", cardBgDark: "#121620", borderDark: "#1a2540", textMainDark: "#f1f5f9",
  textMutedDark: "#64748b",
  accentBlueDark: "#3b82f6", accentGreenDark: "#3b82f6", accentOrangeDark: "#f59e0b",
  kpi1: "linear-gradient(135deg," + "#003d8f" + "," + "#002a70" + ")",
  ...
  kpi5: "linear-gradient(135deg,#1a5fb5,#0d3d80)"
};

var STATUS_MAP = typeof STATUS_MAP !== "undefined" ? STATUS_MAP : {
  analise:   { label: "Em análise", cor: "#d97706", emoji: "🟡" },
  aguardando:{ label: "Aguardando complementação", cor: "#7c3aed", emoji: "🟣" },
  aprovado:  { label: "Aprovado p/ pagamento", cor: "#1d6fc4", emoji: "🟢" },
  pago:      { label: "Pago", cor: "#0f172a", emoji: "⚫" },
  devolvido: { label: "Devolvido", cor: "#dc2626", emoji: "🔴" }
};

function resolveTipoKey(tipo) {
  if (!tipo) return "padrao";
  const s = String(tipo).trim().toLowerCase();
  if (typeof TINFO !== "undefined" && TINFO[s]) return s;
  if (s === "anexo ii" || s === "anexo 2" || s === "padrao") return "padrao";
  if (s.includes("eng")) return "eng";
  if (s.includes("tfd") || s.includes("tdf")) return "tdf";
  if (s.includes("passag")) return "passagem";
  if (s.includes("pnab") || s.includes("aldir")) return "pnab";
  return "padrao";
}
...
if (typeof window !== "undefined") {
  ...
  window.CHK = CHK;
  window.TINFO = TINFO;
  window.COL_CANON = COL_CANON;
  window.FOOTER_TXT = FOOTER_TXT;
  window.MUN = MUN;
  window.T = T;
  window.STATUS_MAP = STATUS_MAP;
  window.resolveTipoKey = resolveTipoKey;
}

if (typeof module !== "undefined" && module.exports) {
  module.exports = {
    MESES, CHK, TINFO, COL_CANON, FOOTER_TXT, MUN, T, STATUS_MAP, resolveTipoKey
  };
}
