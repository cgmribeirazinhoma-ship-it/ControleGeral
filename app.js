...
async function gerarRelatorioPDF(processos, mesAno, appConfig) {
  ...
  doc.text("CONTROLADORIA GERAL DO MUNICÍPIO - CGM", W/2, 40, { align: "center" });
  doc.setLineWidth(0.5); doc.line(19, 43, W-19, 43);

  doc.text(tituloRelatorio, W/2, 52, { align: "center" });
  // ── Sumário ──
  let y = 62;
...
async function gerarPDF(d, tipo, deferir, checklist, sits) {
  try {
    ...
    // Separador duplo azul abaixo do cabeçalho institucional (mesmo azul do ateste)
    doc.setDrawColor(10, 40, 100); doc.setLineWidth(0.85);
    doc.line(CML, y, CML + CCW, y); y += 2.5;
    doc.setLineWidth(0.22);
    doc.line(CML, y, CML + CCW, y); y += 7;

    // Banner azul preenchido "PROCESSO DE PAGAMENTO" (mesmo azul do ateste)
    ...
    for (let ri = 0; ri < capaRows.length; ri++) {
      ...
      doc.setFillColor(ri % 2 === 0 ? 230 : 242, ri % 2 === 0 ? 239 : 248, ri % 2 === 0 ? 255 : 255);
      doc.setDrawColor(185, 208, 238); doc.setLineWidth(0.2);
    ...
    // ── Barra de cabeçalho azul-marinha (full-width) ─────────────────────────
    const HDR_H = 22;
    doc.setFillColor(10, 40, 100);
    doc.rect(0, 0, W, HDR_H, "F");
    try { doc.addImage(window.BRASAO_B64, "PNG", ML2, 2.5, 16, 17, "BH"); } catch(e) {}
    ...
    for (var ri = 0; ri < dtRows.length; ri++) {
      ...
      doc.setDrawColor(185, 208, 238); doc.line(ML2 + DT_LW, y, ML2 + DT_LW, y + rH2);
      doc.setFont("helvetica", "bold"); doc.setFontSize(FS * 0.87); doc.setTextColor(10, 35, 100);
    ...
    // Última linha: CNPJ/CPF | Nº | Tipo | Valor R$
    {
      ...
      doc.setFillColor(210, 226, 250); doc.setDrawColor(185, 208, 238); doc.setLineWidth(0.2);
      ...
      cols4.forEach(function(c, ci) {
        if (ci > 0) { doc.setDrawColor(185,208,238); doc.line(cx4, y, cx4, y + lastH); }
        doc.setFont("helvetica", "bold"); doc.setFontSize(FS * 0.85); doc.setTextColor(10, 35, 100);
    ...
    for (var i = 0; i < checklist.length; i++) {
      ...
      ensureSpace(doc, rH3);
      doc.setFillColor(i % 2 === 0 ? 238 : 248, i % 2 === 0 ? 244 : 252, i % 2 === 0 ? 255 : 255);
      doc.setDrawColor(195, 215, 242); doc.setLineWidth(0.15);
      doc.rect(ML2, y, MW2, rH3, "FD");
      // Badge círculo azul-escuro com número branco (escalado para não sobrepor)
      var bCX = ML2 + 9, bCY = y + rH3 / 2;
      var cR = 3.0 * scale;
      doc.setFillColor(10, 45, 130); doc.circle(bCX, bCY, cR, "F");
    ...
    doc.text("Governador Edison Lob\u00e3o/MA, " + fv(d.data_ateste) + ".", ML2 + MW2 - 4, y, { align: "right" }); y += LH * 1.15;
    ...
    if (y + sigH2 > SAFE) {
      ...
      try { doc.addImage(window.BRASAO_B64, "PNG", ML2, 2.5, 16, 17, "BH2"); } catch(e) {}
      doc.setFont("helvetica","bold"); doc.setFontSize(10.5); doc.setTextColor(255,255,255);
      doc.text("ESTADO DO MARANH\u00c3O", ML2+20, 7.5);
      doc.text("PREFEITURA MUNICIPAL DE GOVERNADOR EDISON LOB\u00c3O", ML2+20, 12.5);
    ...
    for (var pg = 1; pg <= totalPgs; pg++) {
      doc.setPage(pg);
      if (pg === 1) {
        // Capa — barra azul-marinha, texto branco
        doc.setFillColor(10, 40, 100);
...
function DashboardPage({
  ...
  const porMes = useMemo(() => {
    ...
    return Object.entries(m).sort(([a], [b]) => a < b ? -1 : 1).slice(-60).map(([mes, n]) => ({ mes, n }));
  }, [filtered]);
  ...
  const LineChartSVG = ({ data }) => {
    ...
    const W = 700, H = 230, PL = 44, PR = 12, PT = 36, PB = 40;
    ...
    // Índice do ponto máximo — sempre mostra badge vermelho fixo
      ...
      /*#__PURE__*/React.createElement("svg", { viewBox: `0 0 ${W} ${H}`, style: { width: "100%", minWidth: 400, height: H, overflow: "visible" } },
        ...
        [0, Math.round(maxN / 2), maxN].map((v, i) => {
          ...
          return /*#__PURE__*/React.createElement("text", { key: i, x: PL - 8, y: y + 4,
            textAnchor: "end", fontSize: 12, fill: "#64748b", fontWeight: 600 }, v);
        }),
        // area fill
        ...
        data.map((d, i) => {
          ...
          return /*#__PURE__*/React.createElement(React.Fragment, { key: i },
            // Ponto (maior = vermelho e maior)
            /*#__PURE__*/React.createElement("circle", {
              cx: xs[i], cy: ys[i], r: isPeak ? 7 : 5,
              fill: isPeak ? "#e11d48" : "#3b6ef8",
              stroke: dark ? "#1e3528" : "#fff", strokeWidth: 2,
              style: { cursor: "pointer" },
              onMouseEnter: () => setTooltip({ x: xs[i], y: ys[i], val: d.n }),
              onMouseLeave: () => setTooltip(null)
            }),
            // Badge fixo no pico — abaixo do ponto (sempre visível, nunca cortado)
            isPeak && /*#__PURE__*/React.createElement(React.Fragment, null,
              /*#__PURE__*/React.createElement("rect", {
                x: xs[i] - 30, y: ys[i] + 10, width: 60, height: 22, rx: 5,
                fill: "#e11d48"
              }),
              /*#__PURE__*/React.createElement("text", {
                x: xs[i], y: ys[i] + 25, textAnchor: "middle", fontSize: 12,
        ...
        // tooltip hover (posição inteligente: acima se há espaço, senão abaixo)
        tooltip && /*#__PURE__*/React.createElement(React.Fragment, null, (() => {
          const above = tooltip.y > PT + 30;
          return /*#__PURE__*/React.createElement(React.Fragment, null,
            /*#__PURE__*/React.createElement("rect", {
              x: tooltip.x - 32, y: above ? tooltip.y - 36 : tooltip.y + 10,
              width: 64, height: 26, rx: 6,
              fill: dark ? "#1e2d40" : "#0f172a"
            }),
            /*#__PURE__*/React.createElement("text", {
              x: tooltip.x, y: above ? tooltip.y - 19 : tooltip.y + 27,
              textAnchor: "middle", fontSize: 12, fill: "#fff", fontWeight: 700
            }, tooltip.val + " proc.")
          );
        })())
      )
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    /*#__PURE__*/React.createElement("div", { style: { padding: "20px 24px" } },
      ...
      /*#__PURE__*/React.createElement("div", {
        ...
        /*#__PURE__*/React.createElement("button", {
          onClick: async () => {
            let titulo = "RELATÓRIO DE PROCESSOS";
            let periodo = "";
            ...
            if (filtMes) {
              ...
              titulo = `RELATÓRIO ANUAL — EXERCÍCIO ${filtAno}`;
              periodo = filtAno; // gera título customizado no gerarRelatorioPDF
            } else if (filtOrg) {
              titulo = `RELATÓRIO POR ÓRGÃO: ${filtOrg.slice(0,30)}`;
            }

            toast("⏳ Gerando relatório...", "info");
            // Se filtMes existe, usa o comportamento padrão de mês. Caso contrário, passa o título customizado.
      ...
      porMes.length > 0 && /*#__PURE__*/React.createElement("div", {
        style: { background: cardBg, borderRadius: 14, border: `1.5px solid ${bdr}`,
          padding: "20px 24px", marginBottom: 20, overflow: "visible" }
      },
