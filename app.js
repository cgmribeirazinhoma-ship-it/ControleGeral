...
function _buildProcessosFromData(atomRows, blobArr, atomHist, tableHist) {
  const map = new Map();
  
  // 1. Base de Processos (Real)
  ...
  // 2. RESGATE AGRESSIVO: Varre TODAS as fontes de histórico para achar processos sumidos
  const resgatar = (h) => {
    if (!h) return;
    const k = String(h["Processo"] || h["NÚMERO DO DOCUMENTO"] || "").trim();
    if (k && k !== "0" && !map.has(k)) {
      // Reconstrói o registro de busca usando o máximo de dados do histórico
      map.set(k, {
        "NÚMERO DO DOCUMENTO": k,
        "ORGÃO": h["Órgão"] || h["ORGÃO"] || "",
        "FORNECEDOR": h["Fornecedor"] || h["FORNECEDOR"] || "",
        "VALOR": h["Valor"] || h["VALOR"] || "",
        "DATA": h["Data"] || h["DATA"] || "",
        "OBJETO": h["OBJETO"] || h["Objeto"] || "",
        "CNPJ": h["CNPJ"] || "",
        "MODALIDADE": h["MODALIDADE"] || "",
        "CONTRATO": h["CONTRATO"] || "",
        "SECRETARIO": h["SECRETARIO"] || "",
        "_tipoKey": h["TipoKey"] || h["_tipoKey"] || "padrao",
        "_decisao": h["Decisão"] || h["_decisao"] || "",
        "_usuario": h["_usuario"] || "",
        "_sits": h["_sits"] || [],
        "_obs": h["_obs"] || "",
        "_reconstituido": true
  ...
  // Varre histórico atômico, histórico legado e tabela de histórico
  if (Array.isArray(histBlob)) histBlob.forEach(resgatar);
  if (atomHist?.length) atomHist.forEach(r => resgatar(r.value));
  if (Array.isArray(tableHist)) tableHist.forEach(resgatar);
...
function _buildHistoricoFromData(atomProcs, procBlob, atomHist, histBlob, tableHist) {
  ...
  // Função auxiliar para garantir que o campo "Processo" exista e o formato seja consistente
  const ensureHistFormat = (p) => {
    const base = _procToHist(p);
    const num = String(p["NÚMERO DO DOCUMENTO"] || p["Processo"] || "").trim();
    return { ...base, "Processo": num, "NÚMERO DO DOCUMENTO": num };
  };

  // 1. Processos (Base Primária) — Se existe no processo, DEVE estar no histórico
  if (Array.isArray(procBlob)) {
    procBlob.forEach(p => {
      const k = String(p["NÚMERO DO DOCUMENTO"] || "").trim();
      if (k) map.set(k, ensureHistFormat(p));
    });
  }
  if (atomProcs?.length) {
    atomProcs.forEach(r => {
      if (!r.value) return;
      const k = String(r.value["NÚMERO DO DOCUMENTO"] || "").trim();
      if (k) map.set(k, ensureHistFormat(r.value));
    });
  }

  // 2. Histórico legado e tabela indexada (Sobrescreve com dados específicos se existirem)
  const mergeHist = (h) => {
    const k = String(h["Processo"] || h["NÚMERO DO DOCUMENTO"] || "").trim();
    if (!k) return;
    const existing = map.get(k) || {};
    map.set(k, { ...existing, ...h, "Processo": k, "NÚMERO DO DOCUMENTO": k });
  };

  if (Array.isArray(histBlob)) histBlob.forEach(mergeHist);
  if (atomHist?.length) atomHist.forEach(r => r.value && mergeHist(r.value));
  if (Array.isArray(tableHist)) tableHist.forEach(mergeHist);
...
async function loadAllProcessos() {
  const [atomRows, blobArr, atomHist, tableHist] = await Promise.all([
    ST.list("proc_"),
    ST.get("processos"),
    ST.list("hist_"),
    ST.listHistorico()
  ]);
  return _buildProcessosFromData(atomRows, blobArr, atomHist, tableHist);
}
...
async function loadAllCombined() {
  ...
  return {
    processos: _buildProcessosFromData(atomProcs, procBlob, atomHist, tableHist),
  ...
  const handleSalvar = async (retornarId = false) => {
    ...
    try {
      ...
      if (editMode) {
        ...
        idSalvo = await onSave(row, form, user);
      }
...
function App() {
  ...
  const handleSync = useCallback(async () => {
    ...
    if (numsPresentes.size > 0) {
      ...
      const min = Math.max(1, max - 50); // audita os últimos 50
      const faltantes = [];
