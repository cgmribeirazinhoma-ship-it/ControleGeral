...
function ProtocoloPage({ historico = [], processos = [], dark, toast, appConfig }) {
  ...
  // ── Estado ──
  const [modo, setModo]         = React.useState("historico"); // "historico" | "manual"
  const [busca, setBusca]       = React.useState("");
  const [selecionados, setSel]  = React.useState([]);
  const [gerandoPDF, setGer]    = React.useState(false);

  // Campos para protocolo manual
  const campoVazio = { processo:"", orgao:"", fornecedor:"", cnpj:"", nf:"", valor:"", data:"", objeto:"" };
  const [manuais, setManuais]   = React.useState([{ ...campoVazio, _id: Date.now() }]);

  // ── Dados do histórico para seleção ──
  const parseBRL = v => { const s=String(v||"").replace(/\./g,"").replace(",",".").replace(/[^\d.]/g,""); const n=parseFloat(s); return isNaN(n)?0:n; };
  const fmtBRL   = v => (typeof v === "number" ? v : parseBRL(v)).toLocaleString("pt-BR",{style:"currency",currency:"BRL"});

  // Mescla histórico + processos para ter campos completos
  const itensHist = React.useMemo(() => {
    return historico.map(h => {
      ...
      return {
        ...
        nf:        h["DOCUMENTO FISCAL"] || proc?.["DOCUMENTO FISCAL"] || h["Nº"] || "",
  ...
  const toggleTodos = () => {
    if (selecionados.length === filtrados.length) setSel([]);
    else setSel(filtrados.map(i => i._id));
  };

  // ── Campo manual ──
  const updManual = (id, campo, val) =>
    setManuais(prev => prev.map(m => m._id === id ? {...m, [campo]: val} : m));
  const addManual = () =>
    setManuais(prev => [...prev, { ...campoVazio, _id: Date.now() }]);
  const delManual = id =>
    setManuais(prev => prev.filter(m => m._id !== id));

  // ── Gerar PDF ──
  const gerarPDF = async () => {
    const itens = modo === "historico"
      ? itensHist.filter(i => selecionados.includes(i._id))
      : manuais.filter(m => m.processo || m.fornecedor);

    if (itens.length === 0) {
      toast("Selecione ao menos um item para gerar o protocolo.", "warn");
      return;
    }
    setGer(true);
    try {
      ...
      let y = 10;
      if (window.BRASAO_B64) {
        try { doc.addImage(window.BRASAO_B64, "PNG", (W-22)/2, y, 22, 16); } catch {}
        y += 18;
      } else { y += 4; }
      doc.setFont("helvetica","bold"); doc.setFontSize(9);
      doc.text("ESTADO DO MARANHÃO", W/2, y, {align:"center"}); y+=4;
      doc.text("PREFEITURA MUNICIPAL DE GOVERNADOR EDISON LOBÃO", W/2, y, {align:"center"}); y+=4;
      doc.text("CONTROLADORIA DO MUNICÍPIO", W/2, y, {align:"center"}); y+=5;
      doc.setLineWidth(0.5); doc.line(PL, y, W-PR, y); y+=5;

      doc.setFontSize(13); doc.setFont("helvetica","bold");
      doc.text("PROTOCOLO DE DOCUMENTOS", W/2, y, {align:"center"}); y+=7;

      doc.setFontSize(8); doc.setFont("helvetica","normal");
      doc.text(`Data de emissão: ${hoje}`, PL, y);
      doc.text(`Total de itens: ${itens.length}`, W-PR, y, {align:"right"});
      y += 5;
      doc.setLineWidth(0.3); doc.line(PL, y, W-PR, y); y += 4;

      // ── Tabela ──
      const cols = [
        { label:"Nº Processo", w:22, key:"processo" },
        { label:"Órgão",       w:38, key:"orgao"     },
        { label:"Fornecedor",  w:42, key:"fornecedor"},
        { label:"NF/Doc",      w:18, key:"nf"        },
        { label:"Valor",       w:24, key:"valor"     },
      ...
      let cx = PL;
      doc.rect(PL, y, W-PL-PR, rowH, "F");
      cols.forEach(c => {
        doc.text(c.label, cx+1, y+4.8, {maxWidth: c.w-2});
        cx += c.w;
      });
      y += rowH;
      ...
      itens.forEach((item, idx) => {
        if (y > 260) {
          ...
          cx = PL;
          doc.rect(PL, y, W-PL-PR, rowH, "F");
          cols.forEach(c => { doc.text(c.label, cx+1, y+4.8, {maxWidth:c.w-2}); cx+=c.w; });
          y += rowH;
          doc.setTextColor(0,0,0); doc.setFont("helvetica","normal"); doc.setFontSize(7);
        }
        if (idx % 2 === 0) { doc.setFillColor(240,248,240); doc.rect(PL, y, W-PL-PR, rowH, "F"); }
        cx = PL;
        const vals = {
          processo:  String(item.processo  || ""),
          orgao:     String(item.orgao     || "").slice(0,28),
          fornecedor:String(item.fornecedor|| "").slice(0,30),
          nf:        String(item.nf        || ""),
          valor:     item.valor ? fmtBRL(item.valor) : "",
          data:      String(item.data      || ""),
        };
        cols.forEach(c => {
          doc.text(vals[c.key] || "", cx+1, y+4.8, {maxWidth: c.w-2});
          cx += c.w;
        });
        // linhas verticais
        doc.setDrawColor(180,210,180); doc.setLineWidth(0.1);
        let lx = PL;
        cols.forEach(c => { lx+=c.w; doc.line(lx, y, lx, y+rowH); });
        doc.line(PL, y, PL, y+rowH);
        doc.line(W-PR, y, W-PR, y+rowH);
        doc.line(PL, y+rowH, W-PR, y+rowH);
        y += rowH;
      });
      ...
      y += 4;
      doc.setFont("helvetica","bold"); doc.setFontSize(9);
      doc.text(`VALOR TOTAL: ${fmtBRL(totalVal)}`, W-PR, y, {align:"right"});
      y += 10;

      // ── Assinatura ──
      if (y > 255) { doc.addPage(); y = 20; }
      const xAssin = W/2;
      doc.setLineWidth(0.4); doc.line(xAssin-35, y, xAssin+35, y); y+=4;
      doc.setFont("helvetica","bold"); doc.setFontSize(8);
      doc.text(ctrl.nome || "Controlador(a) Geral", xAssin, y, {align:"center"}); y+=4;
      doc.setFont("helvetica","normal"); doc.setFontSize(7.5);
      doc.text(ctrl.cargo || "Controladoria Geral", xAssin, y, {align:"center"}); y+=4;
      if (ctrl.portaria) { doc.text(ctrl.portaria, xAssin, y, {align:"center"}); y+=4; }
      ...
      for (let pg=1; pg<=totalPages; pg++) {
        ...
        doc.text(`Pg ${pg}/${totalPages} — Emitido em ${hoje} — Sistema ControleGeral`, W/2, 292, {align:"center"});
        doc.setTextColor(0,0,0);
      }

      const nomearq = `Protocolo_${new Date().toISOString().slice(0,10)}.pdf`;
      doc.save(nomearq);
      toast(`✅ Protocolo gerado: ${nomearq}`, "ok");
    } catch(e) {
  ...
  // ── Estilos ──
  const inp = {
    background: dark ? "#1a3a28" : "#fff",
    border: "1px solid " + bdr,
    borderRadius: 7, color: txt,
    padding: "5px 9px", fontSize: 12, width:"100%", boxSizing:"border-box"
  };
  const btn = (bg2, col="#fff") => ({
    background: bg2, color: col, border:"none", borderRadius:8,
    padding:"8px 18px", fontWeight:700, fontSize:12.5, cursor:"pointer"
  });

  // ── Render ──
  return /*#__PURE__*/React.createElement("div", {
    style:{ flex:1, background:bg, minHeight:"100vh", padding:"28px 28px 60px" }
  },
    // Título
    /*#__PURE__*/React.createElement("div", { style:{ marginBottom:20 } },
      ...
      /*#__PURE__*/React.createElement("div", { style:{ fontSize:13, color:txt, opacity:.7, marginTop:3 } },
        "Gere protocolos em PDF a partir do histórico ou insira documentos manualmente"
      )
    ),
    // Seletor de modo
    /*#__PURE__*/React.createElement("div", { style:{ display:"flex", gap:10, marginBottom:20 } },
      ["historico","manual"].map(m =>
        /*#__PURE__*/React.createElement("button", {
          key: m,
          onClick: () => setModo(m),
          style: {
            ...btn(modo===m ? MUN.green : (dark?"#1e3528":"#e8f0e8"), modo===m ? "#fff" : txt),
            border: "2px solid " + (modo===m ? MUN.green : bdr)
          }
        }, m === "historico" ? "🕐 Selecionar do Histórico" : "✏️ Inserir Manualmente")
      )
    ),

    // ── MODO HISTÓRICO ──
    modo === "historico" && /*#__PURE__*/React.createElement("div", null,
      /*#__PURE__*/React.createElement("div", { style:{ display:"flex", gap:10, marginBottom:14, alignItems:"center" } },
        /*#__PURE__*/React.createElement("input", {
        ...
        /*#__PURE__*/React.createElement("button", {
          onClick:toggleTodos,
          style:btn(dark?"#1e3528":"#e8f0e8", txt)
        }, selecionados.length === filtrados.length && filtrados.length > 0 ? "☑️ Desmarcar todos" : "☐ Selecionar todos"),
        /*#__PURE__*/React.createElement("div", { style:{marginLeft:"auto", color:MUN.green, fontWeight:700, fontSize:13} },
          selecionados.length > 0 ? `${selecionados.length} selecionado(s)` : ""
        )
      ),
      // Tabela de seleção
      /*#__PURE__*/React.createElement("div", {
        style:{ background:card, border:"1px solid "+bdr, borderRadius:12, overflow:"hidden", maxHeight:440, overflowY:"auto" }
      },
        filtrados.length === 0
          ...
          : filtrados.map((item, idx) => {
            ...
            return /*#__PURE__*/React.createElement("div", {
              ...
              style:{
                ...
                borderBottom:"1px solid "+bdr,
                borderLeft: sel ? "4px solid "+MUN.green : "4px solid transparent",
                transition:"background .12s"
              }
            },
              /*#__PURE__*/React.createElement("input", {
                type:"checkbox", checked:sel, readOnly:true,
                style:{width:15,height:15,accentColor:MUN.green,flexShrink:0}
              }),
              /*#__PURE__*/React.createElement("div", {
                style:{ display:"grid", gridTemplateColumns:"70px 1fr 1fr 80px 90px 80px", gap:8, flex:1, fontSize:11.5 }
              },
                ...
                /*#__PURE__*/React.createElement("span", { style:{fontWeight:600,color:"#1a7a3a"} }, item.valor ? fmtBRL(item.valor) : ""),
    ...
    modo === "manual" && /*#__PURE__*/React.createElement("div", null,
      manuais.map((m, idx) =>
        /*#__PURE__*/React.createElement("div", {
          key: m._id,
          style:{ background:card, border:"1px solid "+bdr, borderRadius:12,
                  padding:"14px 16px", marginBottom:12, position:"relative" }
        },
          /*#__PURE__*/React.createElement("div", {
            style:{ fontWeight:700, color:MUN.green, marginBottom:10, fontSize:13 }
          }, `Documento #${idx+1}`),
          /*#__PURE__*/React.createElement("div", {
            style:{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10 }
          },
            ...[
              ["processo","Nº Processo","text"],
              ["orgao","Órgão","text"],
              ["fornecedor","Fornecedor","text"],
              ["cnpj","CNPJ/CPF","text"],
              ["nf","Nº NF / Doc. Fiscal","text"],
              ["valor","Valor (R$)","text"],
              ["data","Data","text"],
              ["objeto","Objeto","text"],
            ].map(([campo, label]) =>
              /*#__PURE__*/React.createElement("div", { key:campo },
                /*#__PURE__*/React.createElement("label", {
                  style:{ fontSize:10.5, fontWeight:600, color:txt, opacity:.75, display:"block", marginBottom:3 }
                }, label),
                /*#__PURE__*/React.createElement("input", {
                  type:"text", value:m[campo], placeholder:label,
                  onChange: e => updManual(m._id, campo, e.target.value),
                  style:inp
                })
              )
            )
          ),
          manuais.length > 1 && /*#__PURE__*/React.createElement("button", {
            onClick: () => delManual(m._id),
            style:{ position:"absolute", top:10, right:12, background:"#dc2626",
      ...
      /*#__PURE__*/React.createElement("button", {
        onClick: addManual,
        style:{ ...btn(dark?"#1e4a30":"#e8f5ec", MUN.green), border:"2px dashed "+MUN.green, width:"100%" }
      }, "+ Adicionar outro documento")
    ),

    // ── Botão gerar PDF ──
    /*#__PURE__*/React.createElement("div", { style:{ marginTop:22, display:"flex", gap:12 } },
      /*#__PURE__*/React.createElement("button", {
        onClick: gerarPDF,
        disabled: gerandoPDF,
        style:{ ...btn(MUN.green), fontSize:14, padding:"10px 28px",
                opacity: gerandoPDF ? .6 : 1 }
