  ...
  const onSaveEdit = useCallback(async (row, form, numOriginal, user) => {
    ...
    try {
      ...
      const [resProc] = await Promise.all([
        ...
        // [FIX-HIST-SYNC] Atualiza cgel_historico igual ao onSave — mantém todas
        // as fontes sincronizadas e elimina divergência entre Busca & Editar e Histórico.
        _sbFetch("HIST_POST", null, {
          num_processo: numStr,
          orgao: hRow["Órgão"] || "",
          fornecedor: hRow["Fornecedor"] || "",
          cnpj: hRow["CNPJ"] || "",
          valor: hRow["Valor"] || "",
          tipo_key: hRow["TipoKey"] || "",
          decisao: hRow["Decisão"] || "",
          status: "analise",
          usuario: hRow["_usuario"] || "",
          dados: hRow
        }).catch(() => {})
      ]);
      ...
      // Isso evita que o refresh leia proc_NUM antigo do Supabase enquanto
      // a fila de escritas ainda não confirmou a gravação.
      try { MEM[`proc_${numStr}`] = novoItem; } catch {}
      try { MEM[`hist_${numStr}`] = hRow; } catch {}
      // Incrementa versão APÓS garantir que o cache local está atualizado.
      // O evento realtime que isso dispara só vai refrescar quando o refresh
      // conseguir rodar — mas savingInProgressRef ainda estará true por 3s.
      _incrementarVersao().catch(()=>{});
      ...
      // refreshes prematuros (realtime + polling) enquanto a fila de escritas
      // do Supabase ainda não confirmou proc_NUM. Após 3s, libera e agenda
      // um refresh que agora lerá os dados corretos.
      setTimeout(() => {
        savingInProgressRef.current = false;
        setTimeout(() => { if (refreshRef.current) refreshRef.current(false); }, 500);
      }, 3000);
