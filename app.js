...
================================================================================
  ...
  Controladoria Geral · Prefeitura Municipal de Gov. Edison Lobão / MA
...
async function gerarRelatorioPDF(processos, mesAno, appConfig) {
  try {
    ...
    function drawHdr(doc) {
      ...
      doc.text("PREFEITURA MUNICIPAL DE GOVERNADOR EDISON LOBÃO", ML2 + 20, 12.5);
      doc.setFontSize(9);
    ...
    doc.text(`Gov. Edison Lobão/MA, ${hoje.getDate()} de ${["","janeiro","fevereiro","março","abril","maio","junho","julho","agosto","setembro","outubro","novembro","dezembro"][hoje.getMonth()+1]} de ${hoje.getFullYear()}`, W-19, y, { align: "right" });
    y += 16;
...
async function gerarPDF(d, tipo, deferir, checklist, sits) {
  try {
    ...
    doc.text("Gov. Edison Lobão/MA, " + fv(d.data_ateste) + ".", ML2 + MW2 - 4, y, { align: "right" }); y += LH * 1.15;
  ...
  const docObj = new Document({
    sections: [{
      children: [new Paragraph({
        ...
        children: [new TextRun({
          text: `Gov. Edison Lobão/MA, ${dtExt(new Date())}`,
          size: 22
        })]
      }), new Paragraph({
        text: ""
...
function Brasao({
  ...
  return /*#__PURE__*/React.createElement("img", {
    src: src,
    alt: "Brasão Pref. Gov. Edison Lobão",
    width: size,
    height: size,
    style: {
      objectFit: "contain",
...
function LoginPage({
  ...
  }, "PREFEITURA MUNICIPAL DE GOVERNADOR EDISON LOBÃO"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 11,
      color: "#93c5fd",
      marginTop: 4
...
function Sidebar({
  ...
  !mini && /*#__PURE__*/React.createElement(React.Fragment, null,
    ...
    }, "Pref. Municipal de Gov. Edison Lobão / MA")
  ),
  /*#__PURE__*/React.createElement("button", {
    onClick: toggleMini,
...
function ProtocoloPage({ historico = [], processos = [], dark, toast, appConfig }) {
  ...
  const docVazio = () => ({ _id: Date.now()+Math.random(), descricao:"", nomeRecebedor:"", local:"Gov. Edison Lobão – MA" });
  ...
  const gerarPDFSistema = async () => {
    ...
    try {
      ...
      const desenharVia = (doc, isSegundaVia) => {
        ...
        doc.text(`Gov. Edison Lobão/MA, ${hoje}`, W-M, y, {align:"right"}); y+=12;
  ...
  const gerarPDFManual = async () => {
    ...
    try {
      ...
      const localReceb= itens[0]?.local || "Gov. Edison Lobão – MA";
