/*
================================================================================
  SISTEMA DE CONTROLE DE PROCESSOS DE PAGAMENTO — React v4.3
...
async function _sbFetch(method, chave, valor) {
  ...
  try {
    if (method === "GET") {
      ...
      return data.length ? data[0].valor : _SB_NOT_FOUND;
    }
    if (method === "POST") {
...
// [FIX-WRITE-QUEUE] Fila de escrita por chave — garante que duas escritas
// simultâneas para a mesma chave (ex: polling + save simultâneos) sejam sempre
// serializadas. Sem isso, duas chamadas ST.set("orgaos_config") poderiam intercalar
// no Supabase, com a mais antiga sobrescrevendo a mais nova.
const _writeQueues = {};
...
const ST = {
    ...
    if (_sbReady) {
      try {
        ...
        if (raw === _SB_NOT_FOUND) {
          // [FIX-SYNC5] Chave genuinamente ausente no Supabase:
          // limpa cache local obsoleto e retorna null (não usa stale localStorage)
        ...
        // raw === null = erro de rede → cai para localStorage (fallback offline)
      } catch {}
    }
    try {
  ...
  async set(k, v) {
    // [FIX-WRITE-QUEUE] Chaves de configuração compartilhada são serializadas por fila
    // para evitar race condition entre escritas concorrentes (ex: 3 usuários editando orgaos)
    const isSingleton = ["users","orgaos_config","app_config","imported_max_num","_versao_banco",
      "processos","historico","draft_form"].some(s => k === s || k.startsWith(s));
    if (isSingleton) {
      return _enqueueWrite(k, () => this._setDirect(k, v));
    }
...
function dtExt(d) {
  ...
  if (digs.length >= 8) {
    ...
    // [BUG-FIX] Valida mes antes de acessar MESES[] para evitar "undefined"
...
function NovoProcessoPage({
  ...
  const handleSalvar = async () => {
    ...
    // [FIX-DUPLICATA-LOCAL] Verificação rápida com estado local
    ...
    setLoading(true);
    try {
      // [FIX-DUPLICATA-REMOTA] Verifica no Supabase se o número já foi ocupado
      // por outro usuário desde o último refresh (uso simultâneo)
      if (!editMode && _sbReady) {
        try {
          const jaExiste = await ST.get(`proc_${String(form.numDoc).trim()}`);
          if (jaExiste) {
            setLoading(false);
            setConfirmModal({
              titulo: "Número já registrado por outro usuário",
              msg: `⚠️ O número ${form.numDoc} foi registrado por outro usuário enquanto você preenchia o formulário.\n\nClique em Confirmar para usar o Nº ${nextProcessoNumber} (próximo disponível).`,
              tipo: "warn",
              onOk: () => {
                setConfirmModal(null);
                upd("numDoc")(String(nextProcessoNumber));
                setForm(f => ({ ...f, numDoc: String(nextProcessoNumber) }));
                toast(`🔢 Número atualizado para ${nextProcessoNumber}`, "info");
              }
            });
            return;
          }
...
// [B4.1] Inicializado com -1: força refresh real na 1ª chamada mas sem loop
let _versaoLocal = -1;

async function _incrementarVersao() {
  try {
    // [FIX-SYNC1] Usa timestamp em vez de contador read-modify-write.
    // Evita race condition quando dois usuários salvam simultaneamente:
    // ambos liam o mesmo valor e só incrementavam 1x, silenciando 1 save.
    const v = Date.now();
    await _sbFetch("POST", "_versao_banco", JSON.stringify(v));
    // [FIX-SYNC-RACE] Reseta _versaoLocal para -1 (sentinel) em vez de v.
    // Problema sem o fix: com 3 usuários salvando ao mesmo tempo, todos chamam
    // _incrementarVersao() e o banco fica com o timestamp de quem chegou por último
    // (ex: t_C). Esse usuário (C) seta _versaoLocal = t_C. No próximo poll,
    // _versaoBancoCambou() lê remota = t_C === _versaoLocal → retorna false → C
    // nunca chama refresh() e nunca vê os saves de A e B.
    // Com _versaoLocal = -1: próximo poll → remota = qualquer timestamp ≠ -1 →
    // sempre retorna true → garante 1 refresh após todo save → pega saves concorrentes.
    _versaoLocal = -1;
  } catch {}
}
...
async function _versaoBancoCambou() {
  ...
  try {
    ...
    if (raw === null) {
      // [FIX-SYNC10] GET retornou null = falha de rede → marca offline
      // Antes: _sbLive nunca era setado false após a conexão inicial,
      // mantendo o indicador verde mesmo após queda de rede
      _sbLive = false;
      return true; // força refresh (usará localStorage como fallback)
    }
    _sbLive = true; // Supabase respondeu (com dado ou sentinel)
    if (raw === _SB_NOT_FOUND) {
      // Banco novo / chave ainda não existe — força primeiro refresh
      return true;
    }
    const remota = JSON.parse(raw);
...
function App() {
  ...
  // [FIX-SYNC-RACE-3] Ref espelho de processos — permite onSaveEdit usar sempre o
  // estado mais recente sem capturá-lo no closure (elimina [processos] das deps).
  const processosRef = useRef([]);
  useEffect(() => { processosRef.current = processos; }, [processos]);
  // [B4.1-08] localStorage lido em useEffect (não em useMemo) — elimina leitura DOM síncrona
  // durante renderização. Valor mantido em ref para uso no cálculo de nextProcessoNumber.
  ...
  }, [importedMaxNum]); // re-lê quando importedMaxNum muda (importação de planilha)
  // [M2] Ref para pausar polling durante modo edição (evita sobrescrever dados editados)
  const editModeRef = useRef(false);
  // [FIX-SYNC4] Ref para disparar refresh imediato ao fechar edição
  ...
  // [M-A3] Service Worker já registrado em index.html — não duplicar aqui

  // [J-M3] PWA manifest e theme-color já declarados no index.html — não duplicar aqui
  ...
  useEffect(() => {
    // [FIX-REFRESH-OVERLAP] Flag para evitar refreshes sobrepostos
    // Se a rede estiver lenta e um refresh demorar >20s, o próximo ciclo não inicia outro
    let _refreshInProgress = false;
    const refresh = async (isFirst = false) => {
      // [M2] Não atualiza se o usuário está editando um processo
      if (editModeRef.current) return;
      if (_refreshInProgress && !isFirst) return; // evita sobreposição
      _refreshInProgress = true;
      try {
        // [FIX-CONC1] loadAllCombined: 4 chamadas paralelas em vez de 6
        // (proc_ e processos não são mais duplicados entre loadAllProcessos e loadAllHistorico)
        ...
        const atrasados = (h || []).filter(hh => {
          // [R2 FIX] verifica _decisao (campo interno) E Decisão (campo derivado)
          // para evitar badge falso positivo em processos já decididos
        ...
        _refreshInProgress = false; // [FIX-REFRESH-OVERLAP] libera para o próximo ciclo
      }
    };
    refresh(true); // primeira carga — mostra indicador e erro de rede se necessário
    refreshRef.current = refresh; // [FIX-SYNC4] expõe para onEditModeChange
    // [v4.0] ETag: só recarrega se versão mudou — reduz 90% das chamadas
    const interval = setInterval(async () => {
      const mudou = await _versaoBancoCambou();
      // [FIX-SYNC10] Atualiza indicador online a cada ciclo, não só quando há mudança
      // Antes: setSbOnline só era chamado dentro de refresh(), que só rodava
      // quando mudou===true — indicador ficava desatualizado se rede caísse sem saves
      setSbOnline(_sbLive);
  ...
  // [FIX-SYNC-BLOB] salvarProcessos/salvarHistorico NÃO gravam o blob global.
  // Gravar "processos" ou "historico" inteiro cria race condition grave:
  // User A salva proc 100 → começa a gravar blob; User B salva proc 101 → grava blob;
  // User A termina e sobrescreve → proc 101 sumiu. Chaves atômicas proc_/hist_ resolvem isso.
  // Estes métodos apenas atualizam o estado local do React (sem ST.set global).
  ...
  const salvarOrgaos = async o => {
    ...
    // [FIX-SYNC-ORGAOS] Notifica outros clientes sobre mudança de configuração
    _incrementarVersao().catch(()=>{});
  };
  const onSave = useCallback(async (row, form, user) => {
    ...
    // ── Âncora de numeração — localStorage imediato (síncrono) ──
    ...
    if (!isNaN(numInt) && numInt > 0) {
      ...
      if (numInt > currentMaxNum) {
        ...
        ST.set("imported_max_num", numInt).catch(() => {});
      }
    }
    ...
    // [PERF-PARALLEL-SAVE] proc_, hist_, HIST_POST e _versao_banco são todos
    // independentes — salvar em paralelo: 4 operações em ~1x latência de rede.
    // [FIX-SYNC-RACE-2] _incrementarVersao incluído no Promise.all (não mais
    // fire-and-forget separado). Antes: _versaoLocal era setado assincronamente
    // após o retorno de onSave, podendo chegar tarde demais para o polling.
    // Agora: garante que _versaoLocal = -1 antes de onSave retornar.
    const [resPoc] = await Promise.all([
      ST.set(`proc_${numSalvo}`, novoItem),
      ST.set(`hist_${numSalvo}`, hRow),
      // [v4.0] tabela separada cgel_historico (paralelizado)
      _sbFetch("HIST_POST", null, {
        ...
        decisao: hRow["Decisão"]||"", status: form.status || "analise",
        usuario: hRow["_usuario"]||"", dados: hRow
      }).catch(()=>{}),
      // [FIX-SYNC-RACE-2] _incrementarVersao paralelizado com os saves
      _incrementarVersao().catch(()=>{})
    ]);

    // [M-AU2] Audit log — fire-and-forget (não bloqueia o save)
    ST.set(`audit_${numSalvo}_${Date.now()}`, {
      ...
      try {
        const audits = await ST.list("audit_");
        if (audits.length > 200) {
          const sorted = audits.sort((a, b) => a.key.localeCompare(b.key));
          sorted.slice(0, sorted.length - 200).forEach(r => ST.del(r.key).catch(() => {}));
        }
      } catch {}
    }).catch(() => {});

    // [PERF-OPTIMISTIC-UPDATE] Atualiza estado local imediatamente com dados recém-salvos.
    // Elimina loadAllCombined (4 chamadas de rede) após cada save.
    // O polling de 20s garante consistência com outros usuários.
    setProcessos(prev => {
  ...
  const onSaveEdit = useCallback(async (row, form, numOriginal, user) => {
    ...
    if (user?.perfil !== "admin") {
      // [FIX-STALE-CLOSURE] Lê o processo original diretamente do Supabase
      // em vez de usar `processos` do closure (que pode estar desatualizado com 3 usuários)
      let procOriginal = null;
      try {
        procOriginal = await ST.get(`proc_${numStr}`);
      } catch {}
      // Fallback: busca no estado local se Supabase falhou
      // Usa processosRef.current (ref sempre atualizado) em vez de `processos`
      // do closure, eliminando a necessidade de [processos] nas deps do useCallback.
      if (!procOriginal) {
        procOriginal = processosRef.current?.find(p => String(p["NÚMERO DO DOCUMENTO"]) === numStr);
      }
    ...
    // [PERF-PARALLEL-SAVE] proc_, hist_, HIST_POST e _versao_banco em paralelo.
    // [FIX-SYNC-RACE-2] _incrementarVersao incluído no Promise.all.
    const [resProc] = await Promise.all([
      ST.set(`proc_${numStr}`, novoItem),
      ST.set(`hist_${numStr}`, hRow),
      _sbFetch("HIST_POST", null, {
        num_processo: String(hRow["Processo"]||""), orgao: hRow["Órgão"]||"",
        fornecedor: hRow["Fornecedor"]||"", cnpj: hRow["CNPJ"]||"",
        valor: hRow["Valor"]||"", tipo_key: hRow["TipoKey"]||"",
      ...
      // [FIX-SYNC-RACE-2] paralelizado
      _incrementarVersao().catch(()=>{})
    ]);

    // [M-AU2] Audit log — registra edição (fire-and-forget)
    ST.set(`audit_${numStr}_${Date.now()}`, {
      acao: "editar", num: numStr, usuario,
      ts: new Date().toISOString(),
      campos: { orgao: row["ORGÃO"], fornecedor: row["FORNECEDOR"], valor: row["VALOR"], decisao: form.decisao }
    }).then(async () => {
      // [FIX-CONC3] Limpeza probabilística (10%)
      if (Math.random() > 0.1) return;
      try {
        const audits = await ST.list("audit_");
        if (audits.length > 200) {
          ...
          toDelete.forEach(r => ST.del(r.key).catch(() => {}));
        }
      } catch {}
    }).catch(() => {});

    // [PERF-OPTIMISTIC-UPDATE] Atualiza estado local imediatamente — sem loadAllCombined.
    setProcessos(prev => {
  ...
  const handleSync = useCallback(async () => {
    ...
    // [B4.1-04] Batching de 30 itens por vez — previne erro 429 do Supabase
    const runBatch = async (fns, batchSize = 30) => {
      for (let i = 0; i < fns.length; i += batchSize) {
        await Promise.all(fns.slice(i, i + batchSize).map(f => f()));
      }
    ...
    // [FIX-CONC1] Carrega dados frescos em 4 chamadas (em vez de 6)
    const [combined0, o] = await Promise.all([
      loadAllCombined(),
      ST.get("orgaos_config")
    ]);
    const p = combined0.processos;
    const h = combined0.historico;

    // Envia cada processo individualmente (proc_NUM) — em lotes de 30
    const procFns = (p || [])
      .filter(proc => String(proc["NÚMERO DO DOCUMENTO"] || "").trim())
      .map(proc => () => ST.set(`proc_${String(proc["NÚMERO DO DOCUMENTO"]).trim()}`, proc));

    // Envia cada histórico individualmente (hist_NUM) — em lotes de 30
    const histFns = (h || [])
      .filter(hist => String(hist["Processo"] || hist["NÚMERO DO DOCUMENTO"] || "").trim())
      .map(hist => () => ST.set(`hist_${String(hist["Processo"] || hist["NÚMERO DO DOCUMENTO"]).trim()}`, hist));

    await runBatch(procFns);
    await runBatch(histFns);

    // Envia blob de órgãos (pequeno, sem problema de conflito)
    if (o) await ST.set("orgaos_config", o);

    // Recarrega estado com dados confirmados + notifica outros em paralelo
    const [freshCombined] = await Promise.all([
      loadAllCombined(),
      _incrementarVersao().catch(()=>{})
    ]);
    setProcessos(freshCombined.processos || []);
    setHistorico(freshCombined.historico || []);
...
function ProtocoloPage({ historico = [], processos = [], dark, toast, appConfig }) {
  ...
  // ── Carrega histórico de protocolos emitidos ao abrir aba ──
  React.useEffect(() => {
  ...
  const gerarPDFSistema = async () => {
    ...
    try {
      ...
      // [FIX-SYNC3] Recarrega lista imediatamente — antes só atualizava ao trocar aba
      _carregarHistoricoProtocolos().then(h => setHistProtos(h)).catch(()=>{});
