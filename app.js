...
================================================================================
  ...
  Controladoria Geral · Prefeitura Municipal de Ribeirãozinho do Maranhão-MA
...
function _buildProcessosFromData(atomProcs, procBlob, atomHist, histBlob, tableHist) {
  ...
  // [FIX-PRIORITY] Ordem de prioridade (último vence):
  // 1. histBlob — histórico legado (menor prioridade)
  // 2. atomHist — histórico individual
  // 3. tableHist — cgel_historico table
  // 4. procBlob — blob completo do banco (TODOS os processos)
  // 5. atomProcs — proc_ individuais (mais recentes, maior prioridade)
  //    MAS só se atomProcs tiver mais entradas que 0 e o blob tiver sido carregado.
  //    Se atomProcs vem do localStorage (limitado), procBlob já cobre o conjunto completo.
  ...
  // procBlob carregado do banco tem TODOS os processos — base principal
  if (Array.isArray(procBlob)) procBlob.forEach(p => merge(p));

  // atomProcs atualiza/complementa apenas entradas já existentes no map
  // (novos processos cadastrados recentemente e ainda não no blob)
  // Se atomProcs veio do Supabase (> blob), inclui tudo.
  // Se veio do localStorage (< blob), só aplica sobre os que já existem no blob
  // para não sobrescrever o conjunto completo com dados parciais.
  if (Array.isArray(atomProcs)) {
    const blobSize = Array.isArray(procBlob) ? procBlob.length : 0;
    const atomSize = atomProcs.length;
    if (atomSize >= blobSize || blobSize === 0) {
      // atomProcs tem cobertura completa — usa com prioridade total
      atomProcs.forEach(r => merge(r.value));
    } else {
      // atomProcs é parcial (localStorage) — só atualiza entradas existentes
      atomProcs.forEach(r => {
        if (!r || !r.value) return;
        const k = String(r.value["NÚMERO DO DOCUMENTO"] || r.value["Processo"] || "").trim();
        if (k && map.has(k)) {
          // Só sobrescreve se já existe no blob (atualização de dado)
          map.set(k, { ...(map.get(k) || {}), ...r.value });
        } else if (k) {
          // Novo processo ainda não no blob — adiciona mesmo assim
          map.set(k, { ...(map.get(k) || {}), ...r.value });
        }
      });
    }
...
async function _alocarNumeroAtomico(processosAtuais) {
  ...
  for (let t = 0; t < MAX_TENT; t++) {
    ...
    const parsedSeq = parseInt(String(seqBanco || "").trim(), 10);
    const seq = (!isNaN(parsedSeq) && parsedSeq > 0) ? parsedSeq : 0;
...
function _buildMapDataInner(processos) {
  const is2025OrLater = dataStr => {
    ...
    if (s.includes("/")) {
      ...
      const yr = parseInt(s.substring(0, 4), 10);
      return yr >= 2025;
    } else if (s.length === 4 && /^\d{4}$/.test(s)) {
      return parseInt(s, 10) >= 2025;
    }
    return false;
  };
  const dct = (kC, vC) => {
  ...
  return {
    orgaoSecretario: (() => {
      const m = {};
      for (const p of processos) {
        const k = String(p["ORGÃO"] || p["ORGAO"] || "").trim();
        const v = String(p["SECRETARIO"] || "").trim();
        const dataStr = p["DATA"] || p["Data"] || "";
        if (k && v && is2025OrLater(dataStr)) {
          m[k] = v;
        }
      }
      return m;
    ...
    secretarioOrgao: (() => {
      const m = {};
      for (const p of processos) {
        const k = String(p["SECRETARIO"] || "").trim();
        const v = String(p["ORGÃO"] || p["ORGAO"] || "").trim();
        const dataStr = p["DATA"] || p["Data"] || "";
        if (k && v && is2025OrLater(dataStr)) {
          m[k] = v;
        }
...
async function checkLogin(login, senha) {
  ...
  let u = us[cleanedLogin];
  let canonicalLogin = cleanedLogin;
  if (!u && cleanedLogin) {
    const matchKey = Object.keys(us).find(k => k.toLowerCase() === cleanedLogin.toLowerCase());
    if (matchKey) {
      u = us[matchKey];
      canonicalLogin = matchKey;
    }
  }
  if (!u || !u.ativo) return null;
  const match = (await hashSenha(u.salt, senha)) === u.senha;
  return match ? { ...u, login: canonicalLogin } : null;
}
...
async function gerarRelatorioPDF(processos, mesAno, appConfig) {
  try {
    ...
    function drawHdr(doc) {
      ...
      doc.text("PREFEITURA MUNICIPAL DE RIBEIRÃOZINHO DO MARANHÃO-MA", ML2 + 20, 12.5);
      doc.setFontSize(9);
    ...
    // ── Sumário (estilo azul institucional) ──
    doc.setFontSize(10);
    [
      ...
      const rH = 7;
      doc.setFillColor(ri % 2 === 0 ? 235 : 248, ri % 2 === 0 ? 242 : 252, ri % 2 === 0 ? 250 : 255);
      doc.setDrawColor(185, 210, 235); doc.setLineWidth(0.15);
      doc.rect(19, y - 4.5, W - 38, rH, "FD");

      doc.setFont("helvetica","bold"); doc.setTextColor(10, 40, 100);
      doc.text(l, 25, y);
    ...
    // ── Tabela por órgão (estilo azul institucional) ──
    y += 6;
    doc.setFillColor(10, 40, 100);
    ...
    Object.entries(porOrgao).sort(([,a],[,b])=>b.total-a.total).forEach(([org, dados], ri) => {
      ...
      doc.setFillColor(ri % 2 === 0 ? 235 : 248, ri % 2 === 0 ? 242 : 252, ri % 2 === 0 ? 250 : 255);
      doc.setDrawColor(180, 205, 235); doc.setLineWidth(0.2);
    ...
    y += 2;
    doc.setFillColor(210, 230, 250); doc.setDrawColor(180, 205, 235); doc.setLineWidth(0.25);
    doc.rect(19, y - 4.2, W - 38, 7, "FD");
    doc.setFont("helvetica","bold"); doc.setFontSize(9.5); doc.setTextColor(10, 40, 100);
    ...
    // ── Lista de processos (estilo azul institucional) ──
    ...
    doc.text(`Ribeirãozinho do Maranhão-MA, ${hoje.getDate()} de ${["","janeiro","fevereiro","março","abril","maio","junho","julho","agosto","setembro","outubro","novembro","dezembro"][hoje.getMonth()+1]} de ${hoje.getFullYear()}`, W-19, y, { align: "right" });
    y += 16;
    ...
    // ── Rodapé (estilo azul institucional) ──
    ...
    for (let pg = 1; pg <= totalPgs; pg++) {
      ...
      doc.setFillColor(10, 40, 100);  doc.circle(16, 297 - 6, 1.3, "F");
...
async function gerarPDF(d, tipo, deferir, checklist, sits) {
  try {
    ...
    function cabecalho(doc, withLine) {
      ...
      y += 5;
      doc.text("PREFEITURA MUNICIPAL DE RIBEIRÃOZINHO DO MARANHÃO-MA", W / 2, y, {
        align: "center"
      });
      y += 5;
    ...
    const titTxt0 = "PARECER DE VERIFICA\u00c7\u00c3O E AN\u00c1LISE DOCUMENTAL N\u00ba " + fv(d.processo) + " (LIBERA\u00c7\u00c3O PARA PAGAMENTO)";
    const titH0 = 12;

    const dtRows0 = [
      fv(d.orgao), fv(d.objeto), fv(d.orgao), fv(d.fornecedor), fv(d.modalidade), fv(d.contrato)
    ];
    doc0.setFont("helvetica", "normal");
    doc0.setFontSize(FS0 * 0.88);
    let dtH0_sum = 0;
    for (let ri = 0; ri < dtRows0.length; ri++) {
      const vL = doc0.splitTextToSize(dtRows0[ri] || "\u2014", 182 - 46 - 6);
      dtH0_sum += Math.max(MIN_ROW0, vL.length * LH0 + PAD0);
    }
    dtH0_sum += Math.max(MIN_ROW0, LH0 + PAD0); // 4-cols

    doc0.setFont("helvetica", "bold");
    doc0.setFontSize(FS0 * 0.88);
    let ckRowsH0_sum = 0;
    for (let ci = 0; ci < checklist.length; ci++) {
      const cL = doc0.splitTextToSize(checklist[ci], 153.5);
      ckRowsH0_sum += Math.max(MIN_ROW0, cL.length * LH0 + PAD0);
    }

    doc0.setFont("helvetica", "normal");
    doc0.setFontSize(FS0 * 0.90);
    const lbObs0 = d.obs && d.obs.trim() ? doc0.splitTextToSize(d.obs.trim(), 178) : [];

    let total = 0;
    total += 22 + 5; // Cabeçalho + gap
    total += titH0 + 6; // Caixa de título azul
    total += 12.5 + dtH0_sum + 5; // Seção Dados do Processo
    total += 12.5 + ckRowsH0_sum + 5; // Seção Verificação Documental
    total += 12.5 + (LH0 * 2.05) + (lbObs0.length * LH0) + 16; // Seção Observação + Deferimento
    total += 12.5 + (LH0 * 3.9 + 15); // Seção Assinatura
    total += 12; // Rodapé

    // ── Fator de escala ───────────────────────────────────────────────────────
    // Tenta caber em 1 página de forma harmoniosa, respeitando margens e sem cortar
    const AVAIL_P2 = H - 14;
    let scale = total > AVAIL_P2 ? Math.max(AVAIL_P2 / total, 0.68) : 1.0;
    const FS = FS0 * scale;
    const LH = LH0 * scale;
    ...
    // Separador duplo azul abaixo do cabeçalho institucional
    doc.setDrawColor(10, 40, 100); doc.setLineWidth(0.85);
    doc.line(CML, y, CML + CCW, y); y += 2.5;
    doc.setLineWidth(0.22);
    doc.line(CML, y, CML + CCW, y); y += 7;

    // Banner azul preenchido "PROCESSO DE PAGAMENTO"
    doc.setFillColor(10, 40, 100);
    ...
    // Campos — Com caixas e fundo alternado (estilo azul institucional)
    const capaRows = [
      ["Org\u00e3o:", fv(d.orgao)],
    ...
    const C_LW = 36; // Largura do label ajustada para dar 130mm de área útil ao valor
    for (let ri = 0; ri < capaRows.length; ri++) {
      const lbl = capaRows[ri][0], val = capaRows[ri][1];
      // Definir fonte e tamanho EXATOS da renderização antes de calcular a quebra de texto
      doc.setFont("helvetica", "normal");
      doc.setFontSize(10.0);
      const valW = CCW - C_LW - 6;
      const vL = doc.splitTextToSize(val || "\u2014", valW);
      const valLH = 4.8;
      const rH = Math.max(9.0, vL.length * valLH + 3.2);
      ...
      // Texto do label (negrito azul) — centralizado verticalmente na célula
      doc.setFont("helvetica", "bold"); doc.setFontSize(10.0); doc.setTextColor(10, 40, 100);
      doc.text(lbl, CML + 2.5, y + rH / 2 + 1.2);
      
      // Texto do valor (normal cinza escuro) — centralizado verticalmente
      doc.setFont("helvetica", "normal"); doc.setTextColor(20, 20, 20);
      const vStartY = y + (rH - (vL.length - 1) * valLH) / 2 + 1.2;
      vL.forEach(function(l, li) { doc.text(l, CML + C_LW + 3, vStartY + li * valLH); });
    ...
    function drawHeaderP2() {
      doc.setFillColor(10, 40, 100);
      doc.rect(0, 0, W, HDR_H, "F");
      try { doc.addImage(window.BRASAO_B64, "PNG", ML2, 1.5, 20, 17, "BH_P" + doc.internal.getNumberOfPages()); } catch(e) {}
      doc.setFont("helvetica", "bold"); doc.setFontSize(10.5); doc.setTextColor(255, 255, 255);
      doc.text("ESTADO DO MARANH\u00c3O", ML2 + 22, 7.5);
      doc.text("PREFEITURA MUNICIPAL DE RIBEIR\u00c3OZINHO DO MARANH\u00c3O-MA", ML2 + 22, 12.5);
      doc.setFontSize(9);
      doc.text("CONTROLADORIA GERAL DO MUNIC\u00cdPIO - CGM", ML2 + 22, 17.5);
    }

    function ensureSpace2(needed) {
      if (y + needed > SAFE - 12) {
        doc.addPage();
        drawHeaderP2();
        y = HDR_H + 7 * scale;
    ...
    // ── Barra de cabeçalho azul (full-width) ─────────────────────────
    drawHeaderP2();
    y = HDR_H + 5 * scale;

    // ── Caixa de título azul (completa de um lado ao outro da página em 1 linha só) ─
    doc.setFont("helvetica", "bold");
    const titRaw = "PARECER DE VERIFICA\u00c7\u00c3O E AN\u00c1LISE DOCUMENTAL N\u00ba " + fv(d.processo) + " (LIBERA\u00c7\u00c3O PARA PAGAMENTO)";
    // Garantir rigorosamente em 1 linha só com margem de segurança
    const maxTitW = 190;
    let titFS = Math.min(10.0, FS * 0.92);
    doc.setFontSize(titFS);
    let titW = doc.getTextWidth(titRaw);
    if (titW > maxTitW) {
      titFS = titFS * (maxTitW / titW);
      doc.setFontSize(titFS);
    }
    const titH = Math.max(11.5 * scale, titFS * 0.35 + 8 * scale);

    doc.setFillColor(10, 40, 100);
    doc.rect(0, y, W, titH, "F"); // Faixa azul completa de uma ponta à outra da página (0 a W)

    doc.setTextColor(255, 255, 255);
    const titBase = y + titH / 2 + titFS * 0.35;
    doc.text(titRaw, W / 2, titBase, { align: "center" });
    y += titH + 6 * scale;

    // ── Helper: cabeçalho de seção ────────────────────────────────────────────
    function secHeader2(label, ic) {
      ensureSpace2(15 * scale);
      const iS = 8.5 * scale;
      // Traço curto esquerdo
      doc.setDrawColor(10, 40, 100); doc.setLineWidth(0.6);
      doc.line(ML2, y + iS / 2, ML2 + 4, y + iS / 2);
      // Quadrado colorido do ícone
      const iX = ML2 + 5.5, iY = y;
      ...
      var t = ic[3];
      if (t === "doc") {
        doc.line(iX + 1.8 * scale, iY + 2.6 * scale, iX + iS - 1.8 * scale, iY + 2.6 * scale);
        doc.line(iX + 1.8 * scale, iY + 4.5 * scale, iX + iS - 1.8 * scale, iY + 4.5 * scale);
        doc.line(iX + 1.8 * scale, iY + 6.4 * scale, iX + iS - 1.8 * scale, iY + 6.4 * scale);
      } else if (t === "chk") {
        doc.line(iX + 1.8 * scale, iY + 4.8 * scale, iX + 3.4 * scale, iY + 6.6 * scale);
        doc.line(iX + 3.4 * scale, iY + 6.6 * scale, iX + 6.8 * scale, iY + 2.2 * scale);
      } else if (t === "talk") {
        doc.setDrawColor(255, 255, 255); doc.setLineWidth(0.55);
        doc.roundedRect(iX + 1.2 * scale, iY + 1.2 * scale, iS - 2.4 * scale, 4.8 * scale, 1.0 * scale, 1.0 * scale, "S");
        doc.line(iX + 2.6 * scale, iY + 6.0 * scale, iX + 1.8 * scale, iY + 7.6 * scale);
      } else { // pen
        doc.line(iX + 3.0 * scale, iY + 5.2 * scale, iX + 6.5 * scale, iY + 1.7 * scale);
        doc.line(iX + 3.9 * scale, iY + 6.1 * scale, iX + 7.4 * scale, iY + 2.6 * scale);
        doc.line(iX + 6.5 * scale, iY + 1.7 * scale, iX + 7.4 * scale, iY + 2.6 * scale);
        doc.line(iX + 1.3 * scale, iY + 7.0 * scale, iX + 3.0 * scale, iY + 5.2 * scale);
        doc.line(iX + 1.3 * scale, iY + 7.0 * scale, iX + 3.9 * scale, iY + 6.1 * scale);
        doc.line(iX + 0.9 * scale, iY + 7.4 * scale, iX + 3.5 * scale, iY + 7.4 * scale);
      }
      // Label negrito azul
      doc.setFont("helvetica", "bold"); doc.setFontSize(Math.max(8.5, 10.5 * scale)); doc.setTextColor(10, 40, 100);
      doc.text(label, ML2 + 16.5, y + iS / 2 + 1.2 * scale);
      const lw2 = doc.getTextWidth(label);
      // Traço longo direito até a margem direita da página
      doc.setDrawColor(10, 40, 100); doc.setLineWidth(0.6);
      doc.line(ML2 + 16.5 + lw2 + 2, y + iS / 2, ML2 + MW2, y + iS / 2);
      y += iS + 3.8 * scale;
    }

    // ── DADOS DO PROCESSO ─────────────────────────────────────────────────────
    secHeader2("Dados do Processo", [10, 40, 100, "doc"]);
    ...
    for (var ri = 0; ri < dtRows.length; ri++) {
      ...
      var vL2 = doc.splitTextToSize(val2 || "\u2014", MW2 - DT_LW - 6);
      var rH2 = Math.max(MIN_ROW, vL2.length * LH + PAD);
      ensureSpace2(rH2);
      doc.setFillColor(ri % 2 === 0 ? 235 : 248, ri % 2 === 0 ? 242 : 252, ri % 2 === 0 ? 250 : 255);
      doc.setDrawColor(180, 205, 235); doc.setLineWidth(0.2);
      doc.rect(ML2, y, MW2, rH2, "FD");
      doc.setDrawColor(180, 205, 235); doc.line(ML2 + DT_LW, y, ML2 + DT_LW, y + rH2);
      doc.setFont("helvetica", "bold"); doc.setFontSize(FS * 0.87); doc.setTextColor(10, 40, 100);
      doc.text(lbl2, ML2 + 2.5, y + rH2 / 2 + (FS * 0.87) * 0.33);
      doc.setFont("helvetica", "normal"); doc.setTextColor(20, 20, 20);
      var startTextY = y + (rH2 - (vL2.length - 1) * LH) / 2 + (FS * 0.88) * 0.33;
      vL2.forEach(function(l, li) { doc.text(l, ML2 + DT_LW + 3, startTextY + li * LH); });
      y += rH2;
    }
    // Última linha: CNPJ/CPF | Nº | Tipo | Valor R$
    {
      ...
      ensureSpace2(lastH);
      doc.setFillColor(210, 230, 250); doc.setDrawColor(180, 205, 235); doc.setLineWidth(0.2);
      doc.rect(ML2, y, MW2, lastH, "FD");
      var valorLimpo = fv(d.valor).replace("R$", "").trim();
      var cols4 = [
        { l: "CNPJ/CPF:", v: fv(d.cnpj),             w: 52 },
        { l: "N\u00ba:",  v: fv(d.nf),                w: 26 },
        { l: "Tipo:",     v: fv(d.tipo_nf||d.tipo_doc), w: 58 },
        { l: "Valor R$:", v: valorLimpo,              w: MW2 - (52 + 26 + 58) }
      ];
      var cx4 = ML2;
      cols4.forEach(function(c, ci) {
        if (ci > 0) { doc.setDrawColor(180,205,235); doc.line(cx4, y, cx4, y + lastH); }
        doc.setFont("helvetica", "bold"); doc.setFontSize(FS * 0.84); doc.setTextColor(10, 40, 100);
        var lblTxt = c.l;
        doc.text(lblTxt, cx4 + 2.5, y + lastH / 2 + LH * 0.35);
        var lw4 = doc.getTextWidth(lblTxt) + 2;
        doc.setFont("helvetica", "normal"); doc.setTextColor(20, 20, 20);
        var maxValW = c.w - lw4 - 3;
        var valStr = c.v || "\u2014";
        var vW = doc.getTextWidth(valStr);
        if (vW > maxValW && maxValW > 8) {
          var subFS = (FS * 0.84) * (maxValW / vW) * 0.95;
          if (subFS < 6.2) {
            valStr = valStr.replace("ELETR\u00d4NICA", "ELET.").replace("Eletr\u00f4nica", "Elet.");
            vW = doc.getTextWidth(valStr);
            subFS = (FS * 0.84) * (maxValW / vW) * 0.95;
          }
          doc.setFontSize(Math.max(5.8, subFS));
        }
        doc.text(valStr, cx4 + lw4 + 1.5, y + lastH / 2 + LH * 0.35);
        doc.setFontSize(FS * 0.84);
        cx4 += c.w;
      });
      y += lastH;
    }
    ...
    var chkW = 5.5 * scale;
    var chkRightPad = 3.5;
    var chkX = ML2 + MW2 - chkRightPad - chkW;
    var itemTextX = ML2 + 16.5;
    var itemTextW = chkX - itemTextX - 3;

    for (var i = 0; i < checklist.length; i++) {
      doc.setFont("helvetica", "bold");
      doc.setFontSize(FS * 0.88);
      var dL2 = doc.splitTextToSize(checklist[i], itemTextW);
      var rH3 = Math.max(MIN_ROW, dL2.length * LH + PAD);
      ensureSpace2(rH3);
      doc.setFillColor(i % 2 === 0 ? 235 : 248, i % 2 === 0 ? 242 : 252, i % 2 === 0 ? 250 : 255);
      doc.setDrawColor(185, 210, 235); doc.setLineWidth(0.15);
      doc.rect(ML2, y, MW2, rH3, "FD");

      // Badge círculo azul com número branco
      var bCX = ML2 + 8, bCY = y + rH3 / 2;
      var cR = 2.8 * scale;
      doc.setFillColor(10, 40, 100); doc.circle(bCX, bCY, cR, "F");
      doc.setFont("helvetica", "bold"); doc.setFontSize(8.0 * scale); doc.setTextColor(255, 255, 255);
      doc.text(String(i + 1), bCX, bCY + 0.9 * scale, { align: "center" });

      // Texto do item — negrito, perfeitamente centralizado verticalmente
      doc.setFont("helvetica", "bold"); doc.setFontSize(FS * 0.88); doc.setTextColor(15, 15, 15);
      var textStartY = y + (rH3 - (dL2.length - 1) * LH) / 2 + (FS * 0.88) * 0.33;
      dL2.forEach(function(l, li) { doc.text(l, itemTextX, textStartY + li * LH); });

      // Checkbox verde (checked) ou vermelho (unchecked)
      var chkY = y + rH3 / 2 - chkW / 2;
      doc.setLineWidth(0.55 * scale);
      if (sits[i]) {
        doc.setFillColor(34, 197, 94); doc.rect(chkX, chkY, chkW, chkW, "F");
        doc.setDrawColor(255, 255, 255);
        doc.line(chkX + 0.2 * chkW, chkY + 0.5 * chkW, chkX + 0.42 * chkW, chkY + 0.74 * chkW);
        doc.line(chkX + 0.42 * chkW, chkY + 0.74 * chkW, chkX + 0.8 * chkW, chkY + 0.25 * chkW);
      } else {
        doc.setFillColor(220, 50, 50); doc.rect(chkX, chkY, chkW, chkW, "F");
        doc.setDrawColor(255, 255, 255);
        doc.line(chkX + 0.22 * chkW, chkY + 0.22 * chkW, chkX + 0.78 * chkW, chkY + 0.78 * chkW);
        doc.line(chkX + 0.78 * chkW, chkY + 0.22 * chkW, chkX + 0.22 * chkW, chkY + 0.78 * chkW);
      }
      y += rH3;
    }
    y += 5 * scale;
    ...
    ensureSpace2(LH * 3 + 6 * scale);
    doc.setFont("helvetica", "normal"); doc.setFontSize(FS * 0.92); doc.setTextColor(40, 40, 40);
    doc.text("Nestes termos:", ML2 + 2, y + LH * 0.7);
    doc.text("Ribeir\u00e3ozinho do Maranh\u00e3o-MA, " + fv(d.data_ateste) + ".", ML2 + MW2, y + LH * 0.7, { align: "right" });
    y += LH * 1.05;
    if (d.obs && d.obs.trim()) {
      doc.setFont("helvetica", "normal"); doc.setFontSize(FS * 0.90);
      var oL2 = doc.splitTextToSize(d.obs.trim(), MW2 - 4);
      oL2.forEach(function(ol) { ensureSpace2(LH + 1); doc.text(ol, ML2 + 2, y + LH * 0.7); y += LH; });
      y += 2.5 * scale;
    } else {
      y += 2.5 * scale;
    }
    var decTxt = deferir ? "DEFERIMOS O PAGAMENTO." : "INDEFERIMOS O PAGAMENTO.";
    ensureSpace2(10 * scale);
    doc.setFont("helvetica", "bold"); doc.setFontSize(FS * 1.02); doc.setTextColor(10, 40, 100);
    doc.text(decTxt, W / 2, y + LH * 0.7, { align: "center" });
    y += LH + 6 * scale;
    ...
    doc.text(ctrlNome, W / 2, y, { align: "center" });
    y += LH * 0.95;
    doc.setFont("helvetica", "normal"); doc.setFontSize(FS * 0.9);
    doc.text(ctrlCargo, W / 2, y, { align: "center" });
    if (ctrlPortaria) {
      y += LH * 0.95;
      doc.text(ctrlPortaria, W / 2, y, { align: "center" });
    }
    ...
    for (var pg = 1; pg <= totalPgs; pg++) {
      doc.setPage(pg);
      if (pg === 1) {
        // Capa — barra azul, texto branco
        doc.setFillColor(10, 40, 100);
        doc.rect(0, H - 12, W, 12, "F");
        // Pin (círculo duplo branco)
        doc.setFillColor(255, 255, 255); doc.circle(16, H - 6, 2.6, "F");
        doc.setFillColor(10, 40, 100);  doc.circle(16, H - 6, 1.3, "F");
        doc.setFont("helvetica", "normal"); doc.setFontSize(7.8); doc.setTextColor(255, 255, 255);
        doc.text(FOOTER_TXT, W / 2 + 4, H - 4, { align: "center" });
      } else {
        // Parecer — rodapé idêntico ao da primeira página (barra azul de ponta a ponta)
        doc.setFillColor(10, 40, 100);
        doc.rect(0, H - 12, W, 12, "F");
        // Pin (círculo duplo branco)
        doc.setFillColor(255, 255, 255); doc.circle(16, H - 6, 2.6, "F");
  ...
  const docObj = new Document({
    sections: [{
      children: [new Paragraph({
        ...
        alignment: AlignmentType.CENTER,
        children: [new TextRun({
          text: "PREFEITURA MUNICIPAL DE RIBEIRÃOZINHO DO MARANHÃO-MA",
          bold: true,
          size: 26
        })]
      }), new Paragraph({
        ...
        children: [new TextRun({
          text: `Ribeirãozinho do Maranhão-MA, ${dtExt(new Date())}`,
          size: 22
        })]
      }), new Paragraph({
        text: ""
...
function KPICard({ label, value, gradient, icon, compact }) {
  const [hov, setHov] = React.useState(false);
  const paddingVal = compact ? "8px 12px" : "18px 20px";
  const rRadius = compact ? 10 : 16;
  const shadowVal = compact 
    ? (hov ? "0 6px 16px rgba(0,0,0,.2)" : "0 2px 8px rgba(0,0,0,.1)")
    : (hov ? "0 12px 32px rgba(0,0,0,.28)" : "0 4px 20px rgba(0,0,0,.15)");
  const transformVal = hov ? "translateY(-2px) scale(1.01)" : "translateY(0) scale(1)";
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    /*#__PURE__*/React.createElement("div", { style: {
      position: "absolute", top: compact ? -10 : -14, right: compact ? -10 : -14,
      width: compact ? 40 : 70, height: compact ? 40 : 70, borderRadius: "50%",
      background: "rgba(255,255,255,.12)"
    }}),
    // Círculo menor
    /*#__PURE__*/React.createElement("div", { style: {
      position: "absolute", bottom: compact ? -6 : -8, left: compact ? -6 : -8,
      width: compact ? 25 : 40, height: compact ? 25 : 40, borderRadius: "50%",
      background: "rgba(255,255,255,.07)"
    }}),
    /*#__PURE__*/React.createElement("div", { style: { fontSize: compact ? 16 : 24, marginBottom: compact ? 2 : 6, lineHeight: 1 }}, icon),
    /*#__PURE__*/React.createElement("div", { style: {
      fontSize: compact ? 18 : 26, fontWeight: 800, lineHeight: 1,
      fontFamily: "'Outfit', 'Inter', sans-serif",
      letterSpacing: "-.02em"
    }}, value),
    /*#__PURE__*/React.createElement("div", { style: {
      fontSize: compact ? 9 : 10, opacity: .8, marginTop: compact ? 2 : 5,
      textTransform: "uppercase", letterSpacing: ".08em",
      fontWeight: 600
    }}, label)
  );
}
function PageHeader({ icon, title, sub, cor = "#1d6fc4", dark, actions }) {
  return /*#__PURE__*/React.createElement("div", {
    style: {
...
function SearchSelect({
  ...
  const filtered = useMemo(() => {
    const q = (localVal || "").trim().toLowerCase();
    const safeOpts = (Array.isArray(options) ? options : []).filter(o => o != null).map(String);
    if (!q) return safeOpts.slice(0, 80);
    return safeOpts.filter(o => o.toLowerCase().includes(q)).slice(0, 80);
  }, [options, localVal]);
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: {
      ...
      background: o === value ? dark ? "#002f6e" : "#eff6ff" : "transparent",
      borderBottom: `1px solid ${dark ? "#0f1a2e" : "#f8fafc"}`
    },
    onMouseEnter: e => e.currentTarget.style.background = dark ? "#005200" : "#eff6ff",
    onMouseLeave: e => e.currentTarget.style.background = o === value ? dark ? "#002f6e" : "#eff6ff" : "transparent"
  }, o)))));
}
function FilterBadge({
  ...
  if (!isFiltered) return /*#__PURE__*/React.createElement("div", { style: { height: 16 } });
  return /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 9.5,
      color: "#7c3aed",
...
function Brasao({
  ...
  if (!src) return /*#__PURE__*/React.createElement("div", {
    ...
    width: "100", height: "100", rx: "18", fill: "#002f6e"
  }), /*#__PURE__*/React.createElement("text", {
    y: ".9em", fontSize: "78", textAnchor: "middle", x: "50"
  }, "\u2696\uFE0F")));
  return /*#__PURE__*/React.createElement("img", {
    src: src,
    alt: "Brasão Pref. Ribeirãozinho do Maranhão-MA",
    width: size,
    height: size,
    style: {
      objectFit: "contain",
...
function ConfirmModal({ msg, titulo, onOk, onCancel, dark, tipo = "warn" }) {
  const cores = {
    ...
    info:    { bg: "#1e3a5f", bd: "#1d6fc4", txt: "#bfdbfe", ico: "ℹ️" }
  };
...
function LoginPage({
  ...
  }, "PREFEITURA MUNICIPAL DE RIBEIRÃOZINHO DO MARANHÃO-MA"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 11,
      color: "#93c5fd",
      marginTop: 4
...
function Sidebar({
  ...
  !mini && /*#__PURE__*/React.createElement(React.Fragment, null,
    ...
    }, "Pref. Municipal de Ribeirãozinho do Maranhão-MA")
  ),
  /*#__PURE__*/React.createElement("button", {
    onClick: toggleMini,
  ...
  /*#__PURE__*/React.createElement("div", {
    ...
    mini
      ? /*#__PURE__*/React.createElement("div", {
          title: diaSemana + ", " + dataStr + " — " + horaStr,
          style: { fontSize: 14, cursor: "default" }
        }, "\u{1F552}")
      : /*#__PURE__*/React.createElement(React.Fragment, null,
          /*#__PURE__*/React.createElement("div", {
            style: { fontSize: 9, fontWeight: 600, color: "rgba(255,255,255,.4)",
                     textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 3 }
          }, "\uD83D\uDCC5 " + diaSemana),
          /*#__PURE__*/React.createElement("div", {
            style: { fontSize: 13, fontWeight: 700, color: "#e2e8f0", letterSpacing: ".03em" }
          }, dataStr),
          /*#__PURE__*/React.createElement("div", {
            style: { fontSize: 18, fontWeight: 800, color: MUN.gold, marginTop: 2,
                     fontFamily: "'Outfit','Inter',monospace", letterSpacing: ".06em" }
          }, horaStr)
        )
  ),

  page === "processos" && /*#__PURE__*/React.createElement("div", {
    style: {
      margin: "8px 10px 0",
      padding: "8px 12px",
    ...
    style: {
      fontSize: 9,
      fontWeight: 800,
      color: formPct === 100 ? "#60a5fa" : formPct > 60 ? "#fbbf24" : "#93c5fd"
    }
    ...
    style: {
      ...
      borderRadius: 4,
      transition: "width .4s",
      background: formPct === 100 ? "#3b82f6" : formPct > 60 ? "#f59e0b" : "#1d6fc4"
    }
  }))), /*#__PURE__*/React.createElement("div", {
    style: {
      margin: "8px 10px 0",
  ...
  }, isOnline ? "\u2601\uFE0F" : "\uD83D\uDD34", /*#__PURE__*/React.createElement("span", { style: { fontSize: 8, fontWeight: 700, color: isOnline ? "#60a5fa" : "#f87171" } }, isOnline ? "Online" : "Offline")))));
}

// Helper case-insensitive para obter o secretário do órgão
function getSecretarioForOrgao(orgaosConfig, orgaoName) {
  if (!orgaoName || !orgaosConfig || typeof orgaosConfig !== "object") return "";
  
  // 1. Match direto
  if (orgaosConfig[orgaoName]?.secretario) return orgaosConfig[orgaoName].secretario;
  
  // 2. Normalização case/trim
  const nameClean = orgaoName.trim().toUpperCase();
  if (orgaosConfig[nameClean]?.secretario) return orgaosConfig[nameClean].secretario;
  
  // 3. Normalização usando a função normalizarOrgao (acentos e abreviações)
  const normName = typeof normalizarOrgao === "function" ? normalizarOrgao(orgaoName) : nameClean;
  if (orgaosConfig[normName]?.secretario) return orgaosConfig[normName].secretario;
  
  // 4. Varredura flexível das chaves
  const foundKey = Object.keys(orgaosConfig).find(k => {
    const kClean = k.trim().toUpperCase();
    if (kClean === nameClean) return true;
    const kNorm = typeof normalizarOrgao === "function" ? normalizarOrgao(k) : kClean;
    if (kNorm === normName) return true;
    return false;
  });
  
  return foundKey ? orgaosConfig[foundKey]?.secretario || "" : "";
}

// Helper case-insensitive para verificar se o órgão está ativo
function isOrgaoAtivo(orgaosConfig, orgaoName) {
  if (!orgaoName || !orgaosConfig || typeof orgaosConfig !== "object") return true;
  const nameClean = orgaoName.trim().toUpperCase();
  if (orgaosConfig[orgaoName]) return orgaosConfig[orgaoName].ativo !== false;
  const foundKey = Object.keys(orgaosConfig).find(k => k.trim().toUpperCase() === nameClean);
  return foundKey ? orgaosConfig[foundKey]?.ativo !== false : true;
}
...
function NovoProcessoPage({
  ...
  const activeOrgaosCount = useMemo(() => mp.allOrgaos.filter(org => orgaosConfig[org]?.ativo !== false).length, [mp.allOrgaos, orgaosConfig]);
  const orgAtivos = useMemo(() => mp.allOrgaos.filter(o => isOrgaoAtivo(orgaosConfig, o)), [mp, orgaosConfig]);
  
  const activeContratos = useMemo(() => {
    return mp.allContratos.filter(c => {
      const match = Object.values(contratosMeta || {}).find(m => m && m.numero === c);
      if (match) return !match.inativo && !match.excluido;
      return true;
    });
  }, [mp.allContratos, contratosMeta]);

  const activeForns = useMemo(() => {
    return mp.allFornecedores.filter(f => {
      const temContrato = Object.values(contratosMeta || {}).some(c => c && c.fornecedor && c.fornecedor.toUpperCase() === f.toUpperCase());
      if (!temContrato) return true;
      return Object.values(contratosMeta || {}).some(c => c && !c.inativo && !c.excluido && c.fornecedor && c.fornecedor.toUpperCase() === f.toUpperCase());
    });
  ...
  const formFromRow = useCallback(row => {
    const org = row["ORGÃO"] || row["ORGAO"] || "";
    const sec = getSecretarioForOrgao(orgaosConfig, org) || row["SECRETARIO"] || row["SECRETÁRIO"] || "";
    return {
      numDoc: String(nextProcessoNumber || proxNumero(processos)),
      dataDoc: todayISO(),
      ...
      obs: "",
      notas: "",
      tipo: (typeof resolveTipoKey === "function" ? resolveTipoKey(row["_tipoKey"] || row["TipoKey"] || row["Tipo"]) : (row["_tipoKey"] || "padrao"))
    };
  }, [processos, nextProcessoNumber, orgaosConfig]);
  ...
  useEffect(() => {
    ...
    const safeTipo = (typeof resolveTipoKey === "function" ? resolveTipoKey(row["_tipoKey"] || row["TipoKey"] || row["Tipo"]) : (row["_tipoKey"] || "padrao"));
    setForm({
    ...
    // [FIX Bug D] Restaurar estado do checklist a partir de _sits salvo
    {
      ...
      const chkLen = (typeof CHK !== "undefined" && CHK[safeTipo] ? CHK[safeTipo].length : 0);
      if (Array.isArray(sits) && sits.length === chkLen && chkLen > 0) {
        setChks({ [safeTipo]: sits });
      } else {
        setChks({});
      }
    }
  ...
  const onOrgChange = v => setForm(f => {
    const isSameOrgao = (v && f.orgao && v.trim().toUpperCase() === f.orgao.trim().toUpperCase());
    const sec = v ? (getSecretarioForOrgao(orgaosConfig, v) || mp.orgaoSecretario[v] || "") : "";
    return {
      ...f,
      orgao: v,
      secretario: sec || (isSameOrgao ? f.secretario : ""),
  ...
  const onFornChange = v => {
    ...
    setForm(f => {
      ...
      const novCnpj = f.cnpj || (meta && meta.cnpj) || mp.fornCnpj[v] || "";
      const novObj = f.objeto || (meta && meta.objeto) || mp.fornObjeto[v] || "";
      const novMod = f.modalidade || (meta && meta.modalidade) || mp.fornModalidade[v] || "";
      const novOrg = f.orgao || (meta && meta.orgao) || "";
  ...
  const onContratoChange = v => {
    ...
    const forn = (meta && meta.fornecedor) || mp.contratoForn[v] || "";
    const org  = (meta && meta.orgao) || mp.contratoOrgao[v] || "";
    const mod  = (meta && meta.modalidade) || mp.contratoModal[v] || "";
    const obj  = (meta && meta.objeto) || mp.contratoObjeto[v] || "";
    const metaCnpj = (meta && meta.cnpj) || "";
    
    setForm(f => {
      const novoFornecedor = forn || f.fornecedor;
      const novoOrgao = org || f.orgao;
      const isSameOrgao = (novoOrgao && f.orgao && novoOrgao.trim().toUpperCase() === f.orgao.trim().toUpperCase());
      const secSug = novoOrgao ? (getSecretarioForOrgao(orgaosConfig, novoOrgao) || mp.orgaoSecretario[novoOrgao] || "") : "";
      return {
        ...f,
        contrato: v,
        ...
        secretario: secSug || (isSameOrgao ? f.secretario : "")
      };
    });
    if (forn || org || meta) {
  ...
  const activeSecsOpts = useMemo(() => {
    const s = new Set();
    Object.values(orgaosConfig || {}).forEach(o => {
      if (o.ativo !== false && o.secretario) s.add(o.secretario);
    });
    return [...s].sort();
  }, [orgaosConfig]);

  const metaC = useMemo(() => {
    if (!form.contrato || !contratosMeta) return null;
    const matchingKey = Object.keys(contratosMeta).find(k => {
      return typeof cleanContrato === "function" ? cleanContrato(k) === cleanContrato(form.contrato) : k === form.contrato;
    });
    return matchingKey ? contratosMeta[matchingKey] : null;
  }, [form.contrato, contratosMeta]);

  const selectedContratoProgress = useMemo(() => {
    if (!form.contrato) return null;
    const valorExecutado = processos
      .filter(p => {
        const c = p.CONTRATO || p.Contrato || "";
        return typeof cleanContrato === "function" ? cleanContrato(c) === cleanContrato(form.contrato) : c === form.contrato;
      })
      .filter(p => p.SITUACAO !== "CANCELADO")
      .reduce((sum, p) => sum + (typeof parseMonetario === "function" ? parseMonetario(p.VALOR || p.Valor || p["Valor Líquido"] || "") : 0), 0);
    ...
    const valorTotal = metaC.valorTotal || 0;
    const aditivos = metaC.aditivos || 0;
    const totalComAditivos = valorTotal + aditivos;
    
    let finalExecutado = valorExecutado;
    if (metaC.valorExecutadoManual !== undefined && metaC.valorExecutadoManual !== null) {
      finalExecutado = metaC.valorExecutadoManual;
    }
    
    const saldo = totalComAditivos - finalExecutado;
    const progresso = totalComAditivos > 0 ? (finalExecutado / totalComAditivos) * 100 : 0;
    return { totalComAditivos, valorExecutado: finalExecutado, saldo, progresso, validade: metaC.validade || "" };
  }, [form.contrato, metaC, processos]);
  
  const mFF = form.fornecedor ? mp.fornModalidadesList[form.fornecedor] || [] : [];
  const mShow = metaC && metaC.modalidade ? [metaC.modalidade] : (modMode === "forn" && mFF.length ? mFF : mp.allModalidades);
  const mFiltered = modMode === "forn" && Boolean(mFF.length);
  const cFF = form.fornecedor ? mp.fornContratosList[form.fornecedor] || [] : [];
  const cShow = contMode === "forn" && cFF.length ? cFF.filter(c => activeContratos.includes(c)) : activeContratos;
  const cFiltered = contMode === "forn" && Boolean(cFF.length);
  const oFF = form.fornecedor ? mp.fornObjetosList[form.fornecedor] || [] : [];
  const oShow = metaC && metaC.objeto ? [metaC.objeto] : (objMode === "historico" && oFF.length ? oFF : mp.allObjsHist);
  const secSug = form.orgao && !form.secretario ? getSecretarioForOrgao(orgaosConfig, form.orgao) || mp.orgaoSecretario[form.orgao] : "";
  const configSec = form.orgao ? getSecretarioForOrgao(orgaosConfig, form.orgao) : "";
  const secsOpts = configSec ? [configSec] : activeSecsOpts;
  const makeDados = () => {
    ...
    const modalidade= form.modalidade|| (metaC ? metaC.modalidade : "") || (useMap ? mp.fornModalidade[forn] || mp.orgaoModalidade[org] || "" : "");
    const secretario= form.secretario|| (useMap ? getSecretarioForOrgao(orgaosConfig, org) || mp.orgaoSecretario[org] || "" : "");
    const objeto    = form.objeto    || (metaC ? metaC.objeto : "") || (useMap ? mp.fornObjeto[forn] || "" : "");
  ...
  const safeTipo = (typeof resolveTipoKey === "function" ? resolveTipoKey(form.tipo) : (form.tipo || "padrao"));
  const ti = (typeof TINFO !== "undefined" && (TINFO[safeTipo] || TINFO[form.tipo])) || (typeof window !== "undefined" && window.TINFO && (window.TINFO[safeTipo] || window.TINFO[form.tipo])) || { label: "Anexo II", icon: "📄", cor: "#3b82f6" };
  const chkItems = (typeof CHK !== "undefined" && (CHK[safeTipo] || CHK[form.tipo])) || (typeof window !== "undefined" && window.CHK && (window.CHK[safeTipo] || window.CHK[form.tipo])) || [];
  const sits = getChks(safeTipo);
  ...
  const tabSt = i => ({
    ...
    border: "none",
    background: "transparent",
    borderBottom: `2px solid ${tab === i ? "#1d6fc4" : "transparent"}`,
    color: tab === i ? "#1d6fc4" : "#9ca3af",
  ...
  fornHistorial.recentes.map((p, i) =>
    ...
    /*#__PURE__*/React.createElement("span", { style:{color:dark?"#60a5fa":"#1d6fc4", fontWeight:600, flexShrink:0, fontSize:11} }, p["VALOR"]||""),
  ...
  draftSaved && /*#__PURE__*/React.createElement("div", {
    style: {
      ...
      display: "flex", alignItems: "center", gap: 6,
      padding: "4px 10px",
      background: draftSaved.cloud ? (dark?"#052e16":"#eff6ff") : (dark?"#1c1400":"#fefce8"),
      borderRadius: 20, border: "1px solid " + (draftSaved.cloud?"#1d6fc433":"#ca8a0433"),
      width: "fit-content"
    }
  },
  /*#__PURE__*/React.createElement("span", {style:{fontSize:10}}, draftSaved.cloud ? "\u2601\uFE0F" : "\uD83D\uDCBE"),
  /*#__PURE__*/React.createElement("span", {style:{color: draftSaved.cloud?"#1d6fc4":"#d97706", fontWeight:600}},
    ...
    label: "\xD3rg\xE3os",
    value: activeOrgaosCount,
    gradient: T.kpi1,
    icon: "\uD83C\uDFDB\uFE0F",
    compact: true
  }), /*#__PURE__*/React.createElement(KPICard, {
    label: "Credores",
    value: activeFornsCount,
    gradient: T.kpi5,
    icon: "\uD83C\uDFE2",
    compact: true
  }), /*#__PURE__*/React.createElement(KPICard, {
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      style: {
        ...
        color: act ? (ti2?.cor || "#1d6fc4") : dark ? "#4a6494" : "#64748b",
        lineHeight: 1.3,
        whiteSpace: "nowrap",
        overflow: "hidden",
        textOverflow: "ellipsis"
...
padding: "10px 16px",
  ...
  /*#__PURE__*/React.createElement(SH, {
    ...
    ? /*#__PURE__*/React.createElement("input", {
        ...
        style: { ...iStyle, marginBottom: 0, background: dark ? "#1a3a2a" : "#f0f7f0",
                 borderColor: "#1d6fc4", color: dark ? "#60a5fa" : "#1565b5",
    ...
    : /*#__PURE__*/React.createElement("div", {
        ...
        style: {
          ...
          borderColor: isManualNum ? bdr : "#1d6fc4",
          color: isManualNum ? tc : (dark ? "#60a5fa" : "#1565b5"),
          fontWeight: 700,
        ...
        style: {
          ...
          border: "1.5px solid " + (isManualNum ? "#ea580c" : "#1d6fc4"),
  ...
  }, isDuplicating ? "\u26A0\uFE0F N\xBA Ordem de Compra" : "N\xBA Ordem de Compra"), /*#__PURE__*/React.createElement("input", {
  ...
  }, [["deferir", "✅ Deferir", "#1d6fc4"], ["indeferir", "❌ Indeferir", "#dc2626"]].map(([v, l, c]) => /*#__PURE__*/React.createElement("label", {
    key: v,
    style: {
      display: "flex",
      alignItems: "center",
  ...
  /*#__PURE__*/React.createElement("div", { style: { display: "flex", gap: 8, flexWrap: "wrap" } },
    Object.entries(typeof STATUS_MAP !== "undefined" ? STATUS_MAP : (window.STATUS_MAP || {})).map(([k, s]) =>
      /*#__PURE__*/React.createElement("button", {
        key: k,
        onClick: () => setForm(f => ({ ...f, status: k })),
        style: {
          padding: "5px 12px", borderRadius: 20, border: `1.5px solid ${form.status === k ? (s?.cor || "#1d6fc4") : bdr}`,
          background: form.status === k ? (s?.cor || "#1d6fc4") + "20" : cardBg,
          color: form.status === k ? (s?.cor || "#1d6fc4") : tc,
...
/*#__PURE__*/React.createElement("label", {
  ...
  }, "\u2611 Checklist \u2014 ", (ti?.label || "Anexo II")), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "grid",
      gridTemplateColumns: "1fr 1fr",
      gap: 4
    ...
    style: {
      ...
      borderRadius: 4,
      transition: "width .3s",
      background: pctChk === 100 ? "#1d6fc4" : "#f59e0b"
    }
  })), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 10,
...
function BuscarPage({
  ...
  // Reset página ao filtrar ou ordenar
  useEffect(() => { setPagAtual(0); }, [dq, filtTipo, filtDec, filtAno, sort]);
  ...
  const renderCelula = (col, proc) => {
    if (col === "_status_badge") {
      ...
      if (dec === "deferir")    return React.createElement("span", { style: { background: "#1e4d8c", color: "#93c5fd", fontSize: 10, fontWeight: 700, borderRadius: 99, padding: "2px 8px", whiteSpace: "nowrap" } }, "✅ Deferido");
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    /*#__PURE__*/React.createElement("select", {
      ...
      Object.entries(typeof TINFO !== "undefined" ? TINFO : (window.TINFO || {})).map(([k, v]) => /*#__PURE__*/React.createElement("option", { key: k, value: k }, v?.label || k))
    ),
    /*#__PURE__*/React.createElement("select", {
  ...
  }, "Exibindo ", exibidos.length, " de ", filtered.length, " \xB7 Total no banco: ", processos.length),
...
function DashboardPage({
  ...
  const _badgeStyle = {
    ...
    borderRadius: 20, background: dark ? "#1e3a28" : "#dbeafe",
    color: dark ? "#60a5fa" : "#1565b5",
    border: `1px solid ${dark ? "#1e4d8c" : "#bfdbfe"}`
  };
  ...
  const LineChartSVG = ({ data, valueKey = "n", isCurrency = false }) => {
    ...
    return /*#__PURE__*/React.createElement("div", { style: { overflowX: "auto", paddingTop: 10 } },
      /*#__PURE__*/React.createElement("svg", { viewBox: `0 0 ${W} ${H}`, style: { width: "100%", minWidth: 400, height: H, overflow: "visible" } },
        ...
        // area fill
        /*#__PURE__*/React.createElement("path", { d: area, fill: isCurrency ? "#3b82f620" : "#1d6fc420" }),
        // line
        /*#__PURE__*/React.createElement("polyline", { points: polyline, fill: "none", stroke: isCurrency ? "#3b82f6" : "#1d6fc4", strokeWidth: 2.5, strokeLinejoin: "round" }),
        // dots + X labels
        data.map((d, i) => {
          ...
          return /*#__PURE__*/React.createElement(React.Fragment, { key: i },
            ...
            /*#__PURE__*/React.createElement("circle", {
              ...
              fill: isPeak ? "#e11d48" : (isCurrency ? "#3b82f6" : "#1d6fc4"),
              stroke: dark ? "#1e2d45" : "#fff", strokeWidth: 2,
    ...
    return /*#__PURE__*/React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 20, flexWrap: "wrap" } },
      /*#__PURE__*/React.createElement("div", { style: { position: "relative" } },
        ...
        pieTooltip && (() => {
          ...
          return /*#__PURE__*/React.createElement("div", {
            ...
            /*#__PURE__*/React.createElement("div", { style: { fontSize: 10, fontWeight: 700, color: dark ? T.accentGreenDark : "#1d6fc4", margin: "2px 0" } }, fmtBRL(s.valor)),
            /*#__PURE__*/React.createElement("div", { style: { fontSize: 10, color: dark ? T.textMutedDark : "#64748b" } }, s?.label || s.key || "")
          );
        })()
      ),
      /*#__PURE__*/React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 6 } },
        data.map(d => /*#__PURE__*/React.createElement("div", {
          ...
          /*#__PURE__*/React.createElement("span", { style: { fontSize: 12, color: tc, fontWeight: pieTooltip === d.key ? 700 : 400 } }, d?.label || d.key || ""),
...
const BarChartCSS = ({ data, onHover }) => {
    ...
    const cores = ["#3b82f6", "#10b981", "#f59e0b", "#ec4899", "#8b5cf6", "#14b8a6", "#ef4444", "#f97316", "#06b6d4"];
    return /*#__PURE__*/React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 8 } },
      data.map((d, i) => /*#__PURE__*/React.createElement("div", {
        ...
        /*#__PURE__*/React.createElement("div", {
          style: { fontSize: 10, color: dark ? "#60a5fa" : "#2563eb", fontWeight: 700,
      ...
      /*#__PURE__*/React.createElement("div", { style: { display: "flex", alignItems: "baseline", gap: 6 } },
        ...
        variacao !== undefined && variacao !== null && /*#__PURE__*/React.createElement("span", {
          style: {
            ...
            color: variacao >= 0 ? (dark ? T.accentGreenDark : "#1d6fc4") : (dark ? "#f87171" : "#dc2626"),
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    /*#__PURE__*/React.createElement("div", { style: { padding: "20px 24px" } },
        ...
        /*#__PURE__*/React.createElement("div", {
          ...
          (filtOrg || filtForn || filtAno || filtMes) && /*#__PURE__*/React.createElement("div", {
            ...
            filtOrg && /*#__PURE__*/React.createElement("span", {
              key: "org", style: {
                background: dark ? "#1e3a5f" : "#dbeafe", border: `1px solid ${dark ? "#1d6fc4" : "#93c5fd"}`,
                borderRadius: 8, padding: "3px 8px", fontSize: 11,
                color: dark ? "#93c5fd" : "#1565b5", display: "inline-flex", alignItems: "center", gap: 4,
                fontWeight: 600
              }
            ...
            filtForn && /*#__PURE__*/React.createElement("span", {
              key: "forn", style: {
                background: dark ? "#1e2d45" : "#dbeafe", border: `1px solid ${dark ? "#3b82f6" : "#93c5fd"}`,
                borderRadius: 8, padding: "3px 8px", fontSize: 11,
                color: dark ? "#93c5fd" : "#1565b5", display: "inline-flex", alignItems: "center", gap: 4,
                fontWeight: 600
              }
      ...
      /*#__PURE__*/React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 14, marginBottom: 16 } },
        ...
        /*#__PURE__*/React.createElement(KPICard, { label: "Total R$", value: totalGeral > 0 ? fmtBRL(totalGeral) : "—", gradient: "linear-gradient(135deg,#2563eb,#047857)", icon: "\uD83D\uDCB0", variacao: comparativo.pctV, variacaoVal: "" })
      ),
        ...
        (comparativo.curN > 0 || comparativo.prvN > 0) && /*#__PURE__*/React.createElement("div", {
          ...
          ...[
            ...
            pct !== null && /*#__PURE__*/React.createElement("span", {
              style: {
                fontSize: 11, fontWeight: 700,
                color: pct >= 0 ? (dark ? T.accentGreenDark : "#1d6fc4") : (dark ? "#f87171" : "#dc2626"),
                background: pct >= 0 ? (dark ? "#052e16" : "#dbeafe") : (dark ? "#1c1c1c" : "#fee2e2"),
                borderRadius: 6, padding: "1px 6px"
              }
            }, pct >= 0 ? "▲" : "▼", Math.abs(pct), "%")
          ))
...
function HistoricoPage({
  ...
  const filtered = useMemo(() => {
    ...
    return r;
  }, [historico, dq, filtDec]);
  // Reset página ao filtrar
  useEffect(() => { setPagAtual(0); }, [dq, filtDec]);
  const totalPags = Math.ceil(filtered.length / PER_PAGE);
  const exibidos = useMemo(() => filtered.slice(pagAtual * PER_PAGE, (pagAtual + 1) * PER_PAGE), [filtered, pagAtual]);
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    return /*#__PURE__*/React.createElement("tr", {
      ...
      style: {
        ...
        alignItems: "center",
        gap: 4,
        background: dark ? "#0f1a0f" : "#eff6ff",
        borderRadius: 5,
        padding: "2px 7px",
        fontSize: 11,
        color: dark ? "#60a5fa" : "#1e4d8c",
        fontWeight: 600
      }
    }, "\uD83D\uDD52 ", h["_registradoEm"]) : /*#__PURE__*/React.createElement("span", {
      ...
      style: {
        ...
        color: isDef ? "#93c5fd" : isIndef ? "#fca5a5" : "#fde68a",
        border: `1px solid ${isDef ? "#1d6fc4" : isIndef ? "#dc2626" : "#ca8a04"}`
      }
...
function UsuariosPage({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: {
      color: u.perfil === "admin" ? "#7c3aed" : "#1d6fc4"
    }
...
function OrgaosPage({
  ...
  const [editOrgName, setEditOrgName] = useState(null);
  ...
  const handleConfirmarExcluir = async () => {
    if (!senhaExcluir.trim()) {
      setDeleteErr("Digite sua senha.");
      return;
    }
    setDeleteLoading(true);
    setDeleteErr("");
    try {
      const ok = await checkLogin(user.login, senhaExcluir.trim());
      if (!ok) {
        setDeleteErr("Senha incorreta. Tente novamente.");
        return;
      }
      const updated = { ...orgaosConfig };
      delete updated[showDeleteModal];
      await ST.set("orgaos_config", updated);
      onOrgaosChange(updated);
      _incrementarVersao().catch(()=>{});
      toast("🗑️ Órgão excluído com sucesso!", "success");
  ...
  const handleSalvarEdicao = async (oldOrgName) => {
    const newName = editOrgNameVal.trim();
    const newSec = editSecVal.trim();
    if (!newName) {
      toast("⚠️ Nome do órgão obrigatório.", "error");
      return;
    }
    const cur = orgaosConfig[oldOrgName] || { ativo: true };
    const updated = { ...orgaosConfig };
    if (newName !== oldOrgName) {
      delete updated[oldOrgName];
    }
    updated[newName] = {
      ...cur,
      secretario: newSec
    ...
    onOrgaosChange(updated);
    setEditOrgName(null);
    // [FIX-SYNC12] Notifica outros clientes da alteração de órgão
    _incrementarVersao().catch(()=>{});
  ...
  React.useEffect(() => {
    ...
    if (localStorage.getItem("auto_inativar_2025_done")) return;
    let count = 0;
    const novos = { ...orgaosConfig };
    const orgaosAtivos2025 = new Set();
    processos.forEach(p => {
      const org = (p["ORGÃO"] || p["ORGAO"] || "").trim();
      const dataStr = p["DATA"] || p["Data"] || "";
      if (dataStr) {
        let is2025 = false;
        if (dataStr.includes("/")) {
          const parts = dataStr.split("/");
          if (parts.length === 3 && parseInt(parts[2], 10) >= 2025) is2025 = true;
        } else if (dataStr.includes("-")) {
          if (dataStr.substring(0, 4) >= "2025") is2025 = true;
        }
        if (is2025 && org) orgaosAtivos2025.add(org);
      }
    });
    allOrgs.forEach(org => {
      if (!orgaosAtivos2025.has(org)) {
        if (!novos[org]) novos[org] = { secretario: "", ativo: true };
        if (novos[org].ativo !== false) {
          novos[org].ativo = false;
          count++;
        }
      }
    });
    if (count > 0) {
      ST.set("orgaos_config", novos).then(() => {
        onOrgaosChange(novos);
        _incrementarVersao().catch(()=>{});
      });
    }
    try { localStorage.setItem("auto_inativar_2025_done", "true"); } catch(e){}
  }, [processos, orgaosConfig, allOrgs, onOrgaosChange]);
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: {
      ...
      padding: "20px 24px"
       ...
       /*#__PURE__*/React.createElement(SH, {
         icon: "\uD83C\uDFDB\uFE0F",
         title: "Filtro de Órgãos (Ativos: " + allOrgs.filter(o => orgaosConfig[o]?.ativo !== false).length + " / Inativos: " + allOrgs.filter(o => orgaosConfig[o]?.ativo === false).length + ")",
         dark: dark
       }),
       /*#__PURE__*/React.createElement("select", {
         value: filtroOrg,
         onChange: e => setFiltroOrg(e.target.value),
         style: { ...iStyle, width: "auto", padding: "4px 8px", fontSize: 12, marginBottom: 12 }
       }, 
         /*#__PURE__*/React.createElement("option", { value: "TODOS" }, "Todos"),
         /*#__PURE__*/React.createElement("option", { value: "ATIVOS" }, "Somente Ativos"),
         /*#__PURE__*/React.createElement("option", { value: "INATIVOS" }, "Somente Inativos")
       )
  ), /*#__PURE__*/React.createElement("div", {
    style: {
      maxHeight: 500,
    ...
    const isAtv = (orgaosConfig[org]?.ativo !== false);
    if (filtroOrg === "ATIVOS") return isAtv;
    if (filtroOrg === "INATIVOS") return !isAtv;
    return true;
  }).map(org => {
    ...
    const secEfetivo = cfg.secretario || mp.orgaoSecretario[org] || "";
    return /*#__PURE__*/React.createElement("div", {
      key: org,
      style: {
        display: "flex",
    ...
    }, editOrgName === org ? /*#__PURE__*/React.createElement("div", {
      style: { flex: 1, display: "flex", flexDirection: "column", gap: 6, marginRight: 10 }
    }, /*#__PURE__*/React.createElement("input", {
      value: editOrgNameVal,
      onChange: e => setEditOrgNameVal(e.target.value),
      placeholder: "Nome do Órgão / Secretaria",
      style: { ...iStyle, padding: "4px 8px", fontSize: 12, marginBottom: 0, fontWeight: "bold" }
    }), /*#__PURE__*/React.createElement("input", {
      value: editSecVal,
      onChange: e => setEditSecVal(e.target.value),
      placeholder: "Secretário(a)",
      style: { ...iStyle, padding: "4px 8px", fontSize: 11, marginBottom: 0 }
    })) : /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 12.5,
        fontWeight: 700,
    ...
    }, org), secEfetivo && /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 11,
        color: "#64748b"
      }
    }, secEfetivo)), /*#__PURE__*/React.createElement("div", {
      style: { display: "flex", gap: 6 }
    }, editOrgName === org ? /*#__PURE__*/React.createElement("button", {
      onClick: () => handleSalvarEdicao(org),
      style: { ...BS("primary", false, dark), height: 30, fontSize: 11, padding: "0 10px" }
    }, "💾 Salvar") : /*#__PURE__*/React.createElement("button", {
      onClick: () => { setEditOrgName(org); setEditOrgNameVal(org); setEditSecVal(secEfetivo); },
      style: { ...BS("secondary", false, dark), height: 30, fontSize: 11, padding: "0 10px" }
    ...
    !ativo && isAdmin && /*#__PURE__*/React.createElement("button", {
      onClick: () => { setShowDeleteModal(org); setSenhaExcluir(""); setDeleteErr(""); },
      style: {
  ...
  })))), showDeleteModal && /*#__PURE__*/React.createElement("div", {
    style: {
      position: "fixed", inset: 0, background: "rgba(0,0,0,.6)",
    ...
    style: {
      ...
      borderRadius: 16, padding: "28px 30px", maxWidth: 400, width: "90%",
      boxShadow: "0 24px 64px rgba(0,0,0,.4)",
      border: `1.5px solid #dc2626`
    },
    onClick: e => e.stopPropagation()
  }, /*#__PURE__*/React.createElement("div", {
    style: { fontSize: 16, fontWeight: 700, color: dark ? "#f8fafc" : "#0f172a", marginBottom: 8 }
  }, "\uD83D\uDDD1\uFE0F Confirmar Exclus\xE3o de \xD3rg\xE3o"),
  /*#__PURE__*/React.createElement("p", {
    style: { fontSize: 13, color: dark ? "#94a3b8" : "#475569", marginBottom: 16, lineHeight: 1.4 }
  }, "Esta a\xE7\xE3o \xE9 irrevers\xEDvel. Para confirmar a exclus\xE3o do \xF3rg\xE3o ", /*#__PURE__*/React.createElement("b", null, showDeleteModal), ", digite sua senha de administrador:"),
  /*#__PURE__*/React.createElement("input", {
    type: "password",
    value: senhaExcluir,
    onChange: e => setSenhaExcluir(e.target.value),
    onKeyDown: e => e.key === "Enter" && !deleteLoading && handleConfirmarExcluir(),
    placeholder: "Sua senha de administrador",
    autoFocus: true,
    style: iStyle
  }),
  deleteErr && /*#__PURE__*/React.createElement("div", {
    style: { fontSize: 12, color: "#ef4444", marginBottom: 12, fontWeight: 600 }
  }, deleteErr),
  /*#__PURE__*/React.createElement("div", {
    style: { display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 16 }
  }, /*#__PURE__*/React.createElement("button", {
    onClick: () => setShowDeleteModal(null),
    style: BS("secondary", false, dark)
  }, "Cancelar"), /*#__PURE__*/React.createElement("button", {
    onClick: handleConfirmarExcluir,
    disabled: deleteLoading,
    style: BS("danger", deleteLoading, dark)
  }, deleteLoading ? "Excluindo..." : "Confirmar Exclus\xE3o")))));
}
...
function ConfigPage({
  ...
  const obterNomeFormatadoBackup = (item) => {
    if (!item) return "";
    const keyName = item.key.replace("backup_", "");
    const ts = item.value?.ts;
    if (ts) {
      ...
      return `Backup de ${dia}/${mes}/${ano} às ${hora}:${min}`;
    }
    if (keyName !== "data") {
      const parts = keyName.split("_");
      if (parts.length === 3) {
        return `Backup de ${parts[2]}/${parts[1]}/${parts[0]}`;
      }
    }
    return keyName === "data" ? "Backup Geral" : `Backup ${keyName}`;
  };
  ...
  React.useEffect(() => {
    ST.list("backup_").then(rows => {
      if (rows) {
        rows.forEach(r => {
          const pLen = r.value?.processos?.length || 0;
          const hLen = r.value?.historico?.length || 0;
          if (pLen === 0 && hLen === 0) {
            ST.del(r.key).catch(() => {});
          }
        });
        const filtrados = rows.filter(r => (r.value?.processos?.length || 0) > 0 || (r.value?.historico?.length || 0) > 0);
        setBackupList(filtrados.sort((a,b) => b.key.localeCompare(a.key)).slice(0,4));
      }
    });
  }, []);
  const handleRestaurarBackup = async (item) => {
    if (!isAdmin) {
      toast("❌ Apenas administradores podem restaurar backups.", "error");
      return;
    }
  ...
  const handleConfirmarRestaurarBackup = async () => {
    if (!confirmBackup) return;
    if (!senhaBackup.trim()) {
      setBackupErr("Digite sua senha.");
      return;
    }
    setBackupLoading(true);
    setBackupErr("");
    try {
      const ok = await checkLogin(user.login, senhaBackup.trim());
      if (!ok) {
        setBackupErr("Senha incorreta. Tente novamente.");
        return;
      }
      const item = confirmBackup.item;
      ...
      setBackupErr("Falha na restauração: " + e.message);
    } finally {
  ...
  // [FIX-BACKUP] Modal de confirmação para restaurar backup com senha
  confirmBackup && /*#__PURE__*/React.createElement("div", {
    style: {
      position: "fixed", inset: 0, background: "rgba(0,0,0,.6)",
      zIndex: 9998, display: "flex", alignItems: "center", justifyContent: "center"
    }
    ...
    style: {
      background: dark ? "#1a2820" : "#fff",
      borderRadius: 16, padding: "28px 30px", maxWidth: 450, width: "90%",
      boxShadow: "0 24px 64px rgba(0,0,0,.4)",
      border: "1.5px solid #dc2626"
    }
  }, /*#__PURE__*/React.createElement("div", {
  ...
  /*#__PURE__*/React.createElement("div", {
    style: { fontSize: 16, fontWeight: 700, color: dark ? "#fca5a5" : "#0f172a",
             textAlign: "center", marginBottom: 8 }
  }, "Restaurar Backup"),
  /*#__PURE__*/React.createElement("p", {
    style: { fontSize: 13, color: dark ? "#e2e8f0" : "#475569",
             lineHeight: 1.7, textAlign: "center", marginBottom: 16, whiteSpace: "pre-line" }
  }, "Deseja restaurar o \"" + obterNomeFormatadoBackup(confirmBackup.item) + "\"?\n\nIsso substituirá todos os dados atuais. Esta ação não pode ser desfeita."),
  /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      flexDirection: "column",
  ...
  }, "🔐 Confirme sua senha de administrador:"), /*#__PURE__*/React.createElement("input", {
    type: "password",
    ...
    onKeyDown: e => e.key === "Enter" && handleConfirmarRestaurarBackup(),
    autoFocus: true,
    style: {
      ...IS(dark),
      width: "100%",
  ...
  backupErr && /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12,
      color: "#dc2626",
      fontWeight: 700,
  ...
  /*#__PURE__*/React.createElement("button", {
    onClick: handleConfirmarRestaurarBackup,
    disabled: backupLoading,
    style: { ...BS("danger", backupLoading, dark), flex: 1, justifyContent: "center", height: 40 }
  }, backupLoading ? "Restaurando..." : "Confirmar")))),
  /*#__PURE__*/React.createElement(PageHeader, {
    icon: "\u2699\uFE0F",
    ...
    style: {
      ...
      borderRadius: 10,
      border: `1.5px solid ${_sbLive ? "#1d6fc4" : _sbReady ? "#ca8a04" : "#ea580c"}`,
      display: "flex",
      alignItems: "center",
      gap: 10,
      fontSize: 13,
  ...
  /*#__PURE__*/React.createElement("div", null,
    /*#__PURE__*/React.createElement("div", {style:{fontWeight:700, color: _sbLive ? "#93c5fd" : _sbReady ? "#fde047" : "#fed7aa"}},
      _sbLive
    ...
    /*#__PURE__*/React.createElement("div", {style:{fontSize:11, color: _sbLive ? "#60a5fa" : _sbReady ? "#fbbf24" : "#fb923c", marginTop:2}},
  ...
  backupList.length === 0
    ...
    : backupList.map(item => /*#__PURE__*/React.createElement("div", {
      ...
      /*#__PURE__*/React.createElement("div", null,
        /*#__PURE__*/React.createElement("div", { style: { fontSize: 13, fontWeight: 600, color: tc } }, obterNomeFormatadoBackup(item)),
...
function CommandPalette({ processos, onClose, onEditar, onNavigate, dark }) {
  ...
  const decBadge = p => {
    if (p["_decisao"] === "deferir")   return { bg:"#1e4d8c", cl:"#93c5fd", tx:"✅ Deferido" };
...
function App() {
  ...
  const verificarEFazerBackup = useCallback(async (p, h) => {
    if (!p || !h || (p.length === 0 && h.length === 0)) {
      console.info("[Backup] Ignorado: sem dados para backup.");
      return;
    }
    try {
      ...
      if (!ultimo || agora - ultimo > 7 * 24 * 60 * 60 * 1000) {
        ...
        const mes = String(dt.getMonth() + 1).padStart(2, "0");
        const ano = dt.getFullYear();
        const keyName = `backup_${ano}_${mes}_${dia}`;
        const data = { processos: p, historico: h, ts: agora };
        await ST.set(keyName, data);
        await ST.set("last_backup_ts", agora);
        console.info("[Backup] Semanal realizado:", keyName);
      }
  ...
  if (!user) return /*#__PURE__*/React.createElement(React.Fragment, null,
    /*#__PURE__*/React.createElement(LoginPage, { onLogin: setUser }),
    window.location.search.includes("test=true") && /*#__PURE__*/React.createElement(TestRunner, {
      user: user,
      setUser: setUser,
  ...
  if (carregando) return /*#__PURE__*/React.createElement(React.Fragment, null,
    /*#__PURE__*/React.createElement("div", {
      style: { minHeight: "100vh", display: "flex", flexDirection: "column",
               alignItems: "center", justifyContent: "center", background: "#003d8f", gap: 18 }
    }, /*#__PURE__*/React.createElement(Brasao, { size: 64 }),
       /*#__PURE__*/React.createElement("div", {
         style: { fontSize: 13, color: "#60a5fa", fontWeight: 600, letterSpacing: ".04em" }
...
/*#__PURE__*/React.createElement(Sidebar, {
    ...
    onEditar: async h => {
      ...
      if (proc) {
        ...
        handleEditar({
          ...
          "_obs": h["_obs"] || "",
          "_sits": h["_sits"] || [],
          "_tipoKey": (typeof resolveTipoKey === "function" ? resolveTipoKey(h["TipoKey"] || h["Tipo"] || h["_tipoKey"]) : (h["TipoKey"] || "padrao")),
          "_decisao": h["_decisao"] || (h["Decisão"] === "DEFERIDO" ? "deferir" : h["Decisão"] === "INDEFERIDO" ? "indeferir" : "")
        });
      }
    },
    ...
    toast: toast,
    appConfig: appConfig
  }), page === "contratos" && /*#__PURE__*/React.createElement(window.ContratosPage || "div", {
    historico: historico,
    processos: processos,
    dark: dark,
    contratosMeta: contratosMeta,
...
function ProtocoloPage({ historico = [], processos = [], dark, toast, appConfig }) {
  ...
  const docVazio = () => ({ _id: Date.now()+Math.random(), descricao:"", nomeRecebedor:"", local:"Ribeirãozinho do Maranhão-MA" });
  ...
  function _cabPDF(doc, W) {
    ...
    doc.text("PREFEITURA MUNICIPAL DE RIBEIRÃOZINHO DO MARANHÃO-MA", W/2, y, {align:"center"}); y+=5;
  ...
  const gerarPDFSistema = async () => {
    ...
    try {
      ...
      const desenharVia = (doc, isSegundaVia) => {
        ...
        if (isSegundaVia) {
          doc.setFillColor(235,242,255); doc.setDrawColor(10,40,100); doc.setLineWidth(0.3);
          doc.roundedRect(M, y, W-2*M, 7, 2, 2, "FD");
          doc.setFont("helvetica","bold"); doc.setFontSize(8); doc.setTextColor(10,40,100);
        ...
        const tHeader = yy => {
          doc.setFillColor(10,40,100); doc.setTextColor(255,255,255);
        ...
        itens.forEach((item,idx)=>{
          ...
          if(idx%2===0){ doc.setFillColor(242,246,255); doc.rect(M,y,W-2*M,rowH,"F"); }
          let cx=M;
          cols.forEach(c=>{
          ...
          doc.setDrawColor(180,205,235); doc.setLineWidth(0.1);
        ...
        doc.text(`Ribeirãozinho do Maranhão-MA, ${hoje}`, W-M, y, {align:"right"}); y+=12;
  ...
  const gerarPDFManual = async () => {
    ...
    try {
      ...
      const localReceb= itens[0]?.local || "Ribeirãozinho do Maranhão-MA";
      ...
      const desenharViaRecebimento = (doc, isSegundaVia) => {
        ...
        itens.forEach((item,idx)=>{
          ...
          doc.setFillColor(idx%2===0?245:252, idx%2===0?248:255, idx%2===0?255:255);
          doc.setDrawColor(10,40,100); doc.setLineWidth(0.35);
          doc.rect(M, y, W-2*M, boxH, "FD");
          doc.setFont("helvetica","bold"); doc.setFontSize(8.5); doc.setTextColor(10,40,100);
  ...
  return /*#__PURE__*/React.createElement("div",{style:{flex:1,background:bg,minHeight:"100vh",padding:"28px 28px 60px"}},
    ...
    /*#__PURE__*/React.createElement("div",{style:{display:"flex",gap:10,marginBottom:20,flexWrap:"wrap"}},
      [
        ...
        /*#__PURE__*/React.createElement("button",{
          ...
          style:{...btn(modo===m?MUN.green:(dark?"#1e2d45":"#e8f0e8"),modo===m?"#fff":txt),
    ...
    modo==="historico" && /*#__PURE__*/React.createElement("div",null,
      /*#__PURE__*/React.createElement("div",{style:{display:"flex",gap:10,marginBottom:14,alignItems:"center",flexWrap:"wrap"}},
        ...
        /*#__PURE__*/React.createElement("button",{onClick:toggleTodos,style:btn(dark?"#1e2d45":"#e8f0e8",txt)},
          ...
          : filtrados.map((item,idx)=>{
              ...
              return /*#__PURE__*/React.createElement("div",{
                ...
                style:{display:"flex",alignItems:"center",gap:12,padding:"10px 14px",cursor:"pointer",userSelect:"none",
                  background:sel?(dark?"#0a3a1a":"#e6f9ed"):(idx%2===0?(dark?"#1e2d45":"#f7fbf7"):card),
                ...
                /*#__PURE__*/React.createElement("div",{style:{display:"grid",gridTemplateColumns:"70px 1fr 1fr 80px 90px 80px",gap:8,flex:1,fontSize:11.5}},
                  ...
                  /*#__PURE__*/React.createElement("span",{style:{fontWeight:600,color:"#1a5faa"}},parseBRL(item.valor||"0")>0?fmtBRL(parseBRL(item.valor)):(item.valor||"")),
          ...
          : filtradosReg.map((reg, idx) => {
              ...
              return /*#__PURE__*/React.createElement("div",{
                ...
                /*#__PURE__*/React.createElement("div",{style:{
                  ...
                  reg.valorTotal > 0 && /*#__PURE__*/React.createElement("div",{style:{fontWeight:700,color:"#1a5faa",fontSize:14,whiteSpace:"nowrap"}},
                  ...
                  (reg.itens||[]).slice(0,5).map((item,ii)=>
                    /*#__PURE__*/React.createElement("div",{
                      ...
                      ehProc ? /*#__PURE__*/React.createElement(React.Fragment,null,
                        ...
                        parseBRL(item.valor||"0")>0&&/*#__PURE__*/React.createElement("span",{style:{fontWeight:600,color:"#1a5faa",whiteSpace:"nowrap"}},fmtBRL(parseBRL(item.valor)))
...
function TestRunner({ user, setUser, setPage, dark, onSave }) {
  const [logs, setLogs] = React.useState([]);
  const [status, setStatus] = React.useState("idle"); // "idle", "running", "success", "fail"
  ...
  React.useEffect(() => {
    ...
    const run = async () => {
      ...
      // Test 1: Autologin
      if (!user) {
        addLog("🔑 Executando Autologin com usuário admin...");
        setUser({ login: "admin", nome: "Administrador de Teste", perfil: "admin" });
        await sleep(1500);
      }
      addLog("✅ Login verificado com sucesso!", "success");

      // Test 2: Dashboard Page
      if (!active) return;
      addLog("🧭 Navegando para o Dashboard...");
      setPage("dashboard");
      await sleep(1500);
      addLog("✅ Dashboard carregado sem falhas!", "success");

      // Test 3: Novo Processo Form
      if (!active) return;
      addLog("🧭 Navegando para a aba Novo Processo...");
      setPage("processos");
      await sleep(1500);
      addLog("✅ Tela Novo Processo carregada!", "success");

      // Test 3.5: Salvar Novo Processo de Teste
      if (!active) return;
      addLog("✍️ Tentando salvar um novo processo de teste no banco de dados...");
      try {
        const testRow = {
          "DATA": "13/07/2026",
          "ORGÃO": "Secretaria de Teste Auto",
          "FORNECEDOR": "Fornecedor Teste Automatizado Ltda",
          "CNPJ": "00.000.000/0001-00",
          "VALOR": "R$ 1.500,00",
          "MODALIDADE": "DISPENSA",
          "CONTRATO": "123/2026",
          "OBJETO": "Prestação de serviços de teste automatizado",
          "DOCUMENTO FISCAL": "Nota Fiscal",
          "Nº": "999",
          "TIPO": "NF",
          "SECRETARIO": "Secretário de Teste",
          "PERÍODO DE REFERÊNCIA": "JULHO/2026",
          "N° ORDEM DE COMPRA": "12345",
          "DATA NF": "13/07/2026"
        };
        const testForm = {
          tipo: "nf",
          decisao: "deferir",
          obs: "Processo de teste gerado pelo robô de validação.",
          status: "analise"
        };
        const numSalvo = await onSave(testRow, testForm, { login: "admin", nome: "Administrador de Teste", perfil: "admin" });
        addLog(`✅ Processo de teste Nº ${numSalvo} salvo com sucesso!`, "success");
      } catch (errSave) {
        addLog(`❌ Falha ao salvar processo: ${errSave.message}`, "error");
      }
      await sleep(1500);

      // Test 4: Buscar Page
      if (!active) return;
      addLog("🧭 Navegando para a aba Buscar & Editar...");
      setPage("buscar");
      await sleep(1500);
      addLog("✅ Busca carregada!", "success");

      // Test 5: Histórico Page
      if (!active) return;
      addLog("🧭 Navegando para a aba Histórico...");
      setPage("historico");
      await sleep(1500);
      addLog("✅ Histórico carregado!", "success");

      // Test 6: Parecer Page
      if (!active) return;
      addLog("🧭 Navegando para a aba Parecer...");
      setPage("parecer");
      await sleep(1500);
      addLog("✅ Aba Parecer carregada!", "success");

      // Test 7: Contratos Page
      if (!active) return;
      addLog("🧭 Navegando para a aba Contratos...");
      setPage("contratos");
      await sleep(1500);
      addLog("✅ Aba Contratos carregada!", "success");

      // Test 8: Usuários Page
      if (!active) return;
      addLog("🧭 Navegando para a aba Usuários...");
      setPage("usuarios");
      await sleep(1500);
      addLog("✅ Aba Usuários carregada!", "success");

      // Test 9: Órgãos Page
      if (!active) return;
      addLog("🧭 Navegando para a aba Órgãos...");
      setPage("orgaos");
      await sleep(1500);
      addLog("✅ Aba Órgãos carregada!", "success");

      // Test 10: Config Page
      if (!active) return;
      addLog("🧭 Navegando para a aba Configurações...");
      setPage("config");
      await sleep(1500);
      addLog("✅ Aba Configurações carregada!", "success");

      if (active) {
        setStatus("success");
        addLog("🎉 TODOS OS TESTES PASSARAM COM SUCESSO!", "success");
      }
    };
    run();
    return () => { active = false; };
  ...
  return /*#__PURE__*/React.createElement("div", {
    style: {
      ...
      border: `2px solid ${status === "success" ? "#10b981" : status === "running" ? "#3b82f6" : "#64748b"}`,
      borderRadius: 12,
    ...
    /*#__PURE__*/React.createElement("div", {
      style: {
        ...
        fontSize: 12,
        marginBottom: 10,
        color: status === "success" ? "#10b981" : status === "running" ? "#3b82f6" : "#64748b",
        display: "flex",
        alignItems: "center",
        justifyContent: "space-between"
      }
    }, 
      /*#__PURE__*/React.createElement("span", null, "⚙️ Painel de Teste de Integração"),
      /*#__PURE__*/React.createElement("span", {
        style: {
          padding: "2px 6px",
          borderRadius: 4,
          background: status === "success" ? "#064e3b" : status === "running" ? "#1e3a8a" : "#334155",
          color: "#fff",
          fontSize: 9
        }
      }, status.toUpperCase())
    ...
    /*#__PURE__*/React.createElement("div", {
      ...
      logs.map((log, i) => 
        /*#__PURE__*/React.createElement("div", {
          ...
          style: {
            marginBottom: 6,
            color: log.type === "success" ? "#10b981" : log.type === "error" ? "#ef4444" : dark ? "#94a3b8" : "#475569"
          }
        }, `[${log.time}] ${log.msg}`)
      )
    ),
