...
function App() {
  ...
  const onSave = useCallback(async (row, form, user) => {
    ...
    // [PERF-PARALLEL-SAVE] proc_ e hist_ são independentes — salvar em paralelo
    // Antes: await proc, depois await hist = 2x latência de rede (~800ms)
    // Agora: ambos em paralelo = ~1x latência (~400ms)
    const [resPoc] = await Promise.all([
      ST.set(`proc_${numSalvo}`, novoItem),
      ST.set(`hist_${numSalvo}`, hRow),
      // [v4.0] tabela separada cgel_historico (fire-and-forget também paralelizado)
    ...
    // Notifica outros usuários (fire-and-forget)
    _incrementarVersao().catch(()=>{});
  ...
  const onSaveEdit = useCallback(async (row, form, numOriginal, user) => {
    ...
    if (user?.perfil !== "admin") {
      ...
      if (!procOriginal) {
        procOriginal = processos.find(p => String(p["NÚMERO DO DOCUMENTO"]) === numStr);
      }
