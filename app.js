...
function App() {
  ...
  const refresh = useCallback(async (isFirst = false) => {
    ...
    if (refreshInProgressRef.current && !isFirst) return;
  ...
  const onSave = useCallback(async (row, form, user) => {
    ...
    try {
      ...
      releaseLock(numSalvo);
      const usuario = user?.login || user?.nome || "sistema";
      const novoItem = { ...row, "_tipoKey": form.tipo, "_decisao": form.decisao, "_obs": form.obs, "_usuario": usuario };
      ...
      if (!isNaN(numInt) && numInt > 0) {
        try {
          ...
          if (numInt > localAtual) localStorage.setItem("cgel_processo_next_anchor", JSON.stringify(numInt));
        } catch {}
        if (numInt > importedMaxNumRef.current) {
      ...
      const [resPoc] = await Promise.all([
        ST.set(`proc_${numSalvo}`, novoItem), ST.set(`hist_${numSalvo}`, hRow),
      ...
      if (resPoc.cloud) toast(`✅ Processo ${row["NÚMERO DO DOCUMENTO"]} salvo na nuvem ☁️`);
      else toast(`⚠️ Processo ${row["NÚMERO DO DOCUMENTO"]} salvo localmente — verifique conexão.`, "warn");
    } finally { savingInProgressRef.current = false; }
  }, [releaseLock, toast]);
