...
function _buildProcessosFromData(atomRows, blobArr, atomHist, tableHist) {
  ...
  // 2. REPARO AUTOMÁTICO: Se existe no Histórico mas não no Processo, reconstrói
  const resgatar = (h) => {
    const k = String(h["Processo"] || h["NÚMERO DO DOCUMENTO"] || "").trim();
    if (k && !map.has(k)) {
      // Reconstrói um objeto de processo mínimo baseado nos dados do histórico
      map.set(k, {
  ...
  if (atomHist?.length) atomHist.forEach(r => r.value && resgatar(r.value));
