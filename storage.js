...
const SUPABASE_URL = _env.SUPABASE_URL || "https://jifuyprnrrmpbmitmzlx.supabase.co";
const SUPABASE_ANON_KEY = _env.SUPABASE_ANON_KEY || "sb_publishable_x-jrFjthVTsco6rwP5VxbA_wMPaUPoO";
...
const DEFAULT_TIMEOUT = 20000; // 20s para queries paginadas grandes
  ...
  try {
    ...
    if (method === "POST") {
      ...
      if (error) {
        fetch('/log', {
          ...
          body: JSON.stringify({ type: 'error', message: 'Supabase cgel_store POST Error (' + chave + '): ' + error.message + ' - ' + JSON.stringify(error) })
        }).catch(()=>{});
      }
    ...
    if (method === "DELETE") {
      ...
      if (error) {
        fetch('/log', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ type: 'error', message: 'Supabase cgel_store DELETE Error (' + chave + '): ' + error.message + ' - ' + JSON.stringify(error) })
        }).catch(()=>{});
      }
      return !error;
    }
    if (method === "LIST") {
      ...
      while (true) {
        ...
        // [FIX-PAGINATE] Retry até 3x por página antes de desistir
        for (let attempt = 0; attempt < 3; attempt++) {
          try {
            const res = await _sb.from('cgel_store')
              .select('chave,valor')
              .like('chave', `${chave}%`)
              .order('chave')
        ...
        if (error || !data) {
          // Se primeira página falhou, retorna null para usar fallback
          if (page === 0) return null;
          // Se páginas posteriores falharam, retorna o que já temos
          console.warn('[storage] LIST parcial: erro na página', page, '- retornando', allData.length, 'itens');
          break;
        }
    ...
    if (method === "HIST_POST") {
      ...
      if (error) {
        fetch('/log', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ type: 'error', message: 'Supabase cgel_historico POST Error: ' + error.message + ' - ' + JSON.stringify(error) })
        }).catch(()=>{});
      }
      return !error;
    }
    ...
    fetch('/log', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ type: 'error', message: 'Supabase Exception (' + method + ' ' + (chave || '') + '): ' + e.message + ' - ' + e.stack })
    }).catch(()=>{});
    ...
    if (_sbReady) {
      try {
        ...
        if (raw === _SB_NOT_FOUND) {
          // Se não está no Supabase, mas temos no localStorage, mantemos e retornamos o local
    ...
    if (_sbReady) {
      try {
        ...
        if (rows !== null) {
          ...
          // Mescla com dados do localStorage para recuperar itens locais/offline
          try {
            for (let i = 0; i < localStorage.length; i++) {
              const k = localStorage.key(i);
              if (k && k.startsWith("cgel_" + prefix)) {
                const keyNoPrefix = k.slice(5);
                if (!sbMap.has(keyNoPrefix)) {
                  const raw = localStorage.getItem(k);
                  if (raw) { try { sbMap.set(keyNoPrefix, JSON.parse(raw)); } catch {} }
                }
              }
            }
          } catch {}
    ...
    if (_sbReady) {
      try {
        ...
        if (rows !== null) {
          return rows.map(r => {
            try {
              const d = typeof r.dados === "string" ? JSON.parse(r.dados) : r.dados;
              if (d) {
