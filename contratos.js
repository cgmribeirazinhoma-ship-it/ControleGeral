...
window.ContratosPage = function ContratosPage({ historico, processos, dark, contratosMeta, setContratosMeta, orgaosConfig, onSaveOrgao, toast, user }) {
  const [editingId, setEditingId] = React.useState(null);
  const [isCreatingNew, setIsCreatingNew] = React.useState(false);
  const [editForm, setEditForm] = React.useState({ valorTotal: 0, validade: "", aditivos: 0 });
  const [filterStatus, setFilterStatus] = React.useState("ativos"); // "ativos" (default), "inativos", "all"
    ...
    (historico || []).forEach(p => {
      ...
      if ((c.endsWith("/2025") || c.endsWith("/2026")) && !ehPequenas(c)) {
        if (!map[c]) {
          ...
          if (!map[c].cnpj) map[c].cnpj = p["CNPJ"] || p["cnpj"] || p["CNPJ/CPF"] || "";
    ...
    return Object.values(map).map(c => {
      ...
      if (validade) {
        ...
        if (parts.length === 3) {
          ...
          if (diasRestantes < 30) corPrazo = "#ef4444"; // red
          else if (diasRestantes < 90) corPrazo = "#f59e0b"; // yellow
        }
      }
  ...
  const orgaosDisponiveis = React.useMemo(() => {
    const s = new Set(uniqueOrgaos);
    if (orgaosConfig && typeof orgaosConfig === 'object') {
      Object.values(orgaosConfig).forEach(o => {
        if (o && o.nome && o.ativo !== false) s.add(o.nome);
      });
    }
    return Array.from(s).sort();
  }, [uniqueOrgaos, orgaosConfig]);
  ...
  const handleSave = async () => {
    ...
    if (onSaveOrgao && editForm.orgao) {
      await onSaveOrgao(editForm.orgao);
    }
    setEditingId(null);
    setIsCreatingNew(false);
  ...
  const handleGerarRelatorio = async () => {
    try {
      ...
      doc.text("PREFEITURA MUNICIPAL DE RIBEIRÃOZINHO DO MARANHÃO-MA", ML + 20, 12.5);
      doc.setFontSize(9);
      ...
      // Subtítulo indicando os filtros ativos
      ...
      filteredList.forEach((c, idx) => {
        // Altura dinâmica de linha com base no fornecedor
        const fText = (c.fornecedor || "—") + (c.cnpj ? " (" + c.cnpj + ")" : "");
        const fLines = doc.splitTextToSize(fText, cols[1].w - 3);
        ...
        if (y + rowH > H - 15) {
          doc.addPage();
          y = 22 + 8;
          // Desenhar cabeçalho da tabela novamente
          doc.setFillColor(10, 40, 100);
          doc.rect(ML, y, W - 2*ML, 8, "F");
          doc.setFont("helvetica", "bold"); doc.setFontSize(7.5); doc.setTextColor(255, 255, 255);
          let curX2 = ML;
        ...
        // Bordas laterais e inferiores finas
        doc.setDrawColor(210, 220, 235); doc.setLineWidth(0.1);
        doc.rect(ML, y, W - 2*ML, rowH, "S");
        
        let cx = ML;
        // Coluna 1: Contrato (se for inativo, coloca um emoji de alerta ou texto)
        ...
        // Cor do saldo dependendo do valor
        doc.setFont("helvetica", "bold");
      ...
      for (let pg = 1; pg <= totalPgs; pg++) {
        ...
        doc.text("Sistema de Controle de Processos de Pagamento — Prefeitura de Ribeirãozinho do Maranhão-MA", W / 2, H - 4, { align: "center" });
  ...
  return React.createElement("div", {
    ...
    React.createElement("div", {
      ...
      [
        ...
        { label: "🟢 Contratos Ativos", val: totalAtivos, color: "#3b82f6" },
    ...
    (editingId || isCreatingNew) && React.createElement("div", {
      ...
      React.createElement("label", { style: { display: "block", marginBottom: 10 } },
        ...
        React.createElement("datalist", { id: "orgaos-list" },
          orgaosDisponiveis.map(o => React.createElement("option", { key: o, value: o }))
        )
      ),
      React.createElement("label", { style: { display: "block", marginBottom: 10 } },
        "Modalidade:",
            ...
            if (val) {
              setPasswordPrompt({
                ...
                onConfirm: async (pwd) => {
                  const isValid = await verifyPassword(pwd);
                  if (isValid) {
                    setEditForm(prev => ({ ...prev, inativo: true }));
                    setPasswordPrompt(null);
                  } else {
                    if (toast) toast("❌ Senha incorreta!", "error");
                    else alert("Senha incorreta!");
              ...
              setEditForm(prev => ({ ...prev, inativo: false }));
            }
          },
          style: { cursor: "pointer", width: 16, height: 16 }
        }),
        "🚫 Inativar Contrato (ocultar nas opções do formulário)"
      ),
      React.createElement("div", { style: { display: "flex", gap: 10, justifyContent: "flex-end" } },
        React.createElement("button", { onClick: () => { setEditingId(null); setIsCreatingNew(false); setRenewedFromId(null); }, style: { padding: "8px 16px", borderRadius: 6, border: "none", background: "#64748b", color: "#fff", cursor: "pointer" } }, "Cancelar"),
        React.createElement("button", { onClick: handleSave, style: { padding: "8px 16px", borderRadius: 6, border: "none", background: "#003d8f", color: "#fff", cursor: "pointer" } }, "Salvar")
      )
    )),
    ...
    React.createElement("div", {
      ...
      React.createElement("div", {
        key: c.id,
        className: "contrato-card card-enter",
        style: {
          background: dark ? "#334155" : "#fff",
          padding: 20, borderRadius: 12,
          boxShadow: "0 4px 6px -1px rgba(0,0,0,0.1)",
          borderTop: `4px solid ${c.inativo ? "#94a3b8" : c.corPrazo}`,
          position: "relative",
          opacity: c.inativo ? 0.65 : 1,
          border: c.inativo ? (dark ? "1px solid #475569" : "1px solid #cbd5e1") : "none"
        }
      },
        React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 } },
          React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8 } },
            React.createElement("h3", { style: { fontSize: 18, margin: 0 } }, c.id),
            c.inativo && React.createElement("span", { 
              style: { 
          ...
          React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8 } },
            c.inativo && user?.perfil === "admin" && React.createElement("button", {
            ...
            React.createElement("button", {
              ...
              style: {
                background: c.inativo ? "#3b82f6" : "#64748b",
                color: "#fff",
                fontSize: 11,
                fontWeight: "bold",
                padding: "4px 10px",
                ...
                alignItems: "center",
                gap: 4,
                boxShadow: c.inativo ? "0 2px 4px rgba(16,185,129,0.2)" : "none"
              }
            }, c.inativo ? "🟢 Reativar" : "🚫 Inativar"),
            React.createElement("button", { 
        ...
        React.createElement("p", { style: { fontSize: 12, opacity: 0.8, marginBottom: 4 } }, c.fornecedor, c.cnpj ? " (CNPJ: " + c.cnpj + ")" : "", " - ", c.orgao),
        React.createElement("p", { style: { fontSize: 11, opacity: 0.7, marginBottom: 6, textTransform: "uppercase" } }, `🏷️ Modalidade: ${c.modalidade || "(Não informada)"}`),
        React.createElement("p", {
          style: {
            ...
            borderRadius: 6,
            fontStyle: "italic",
            borderLeft: `3px solid ${dark ? "#38bdf8" : "#0284c7"}`
          }
        },
          React.createElement("strong", { style: { fontStyle: "normal" } }, "🎯 Objeto: "),
          c.objeto || "(Não cadastrado)"
        ),
        ...
        React.createElement("div", { style: { display: "flex", justifyContent: "space-between", marginBottom: 4, fontSize: 13 } },
          ...
          React.createElement("span", { style: { fontWeight: 600 } }, formatVal(c.valorTotal))
        ),
        React.createElement("div", { style: { display: "flex", justifyContent: "space-between", marginBottom: 4, fontSize: 13 } },
          React.createElement("span", null, "Aditivos:"),
          React.createElement("span", { style: { fontWeight: 600, color: c.alertaAditivo ? "#ef4444" : "inherit" } }, 
            formatVal(c.aditivos), c.alertaAditivo ? " ⚠️ (Lim. 25%)" : ""
          )
        ),
        React.createElement("div", { style: { display: "flex", justifyContent: "space-between", marginBottom: 4, fontSize: 13 } },
          React.createElement("span", null, "Executado:"),
          React.createElement("span", { style: { fontWeight: 600 } }, formatVal(c.valorExecutado))
        ),
        React.createElement("div", { style: { display: "flex", justifyContent: "space-between", marginBottom: 12, fontSize: 13 } },
          React.createElement("span", null, "Saldo Restante:"),
          React.createElement("span", { style: { fontWeight: 800, color: c.saldo <= 0 ? "#ef4444" : "#3b82f6" } }, formatVal(c.saldo))
        ),
        
        React.createElement("div", { style: { height: 6, background: dark ? "#1e293b" : "#e2e8f0", borderRadius: 3, overflow: "hidden", marginBottom: 4 } },
          React.createElement("div", { 
            style: { 
              height: "100%", 
              width: `${Math.min(c.progresso, 100)}%`, 
              background: c.progresso > 90 ? "#ef4444" : c.progresso > 70 ? "#f59e0b" : "#3b82f6" 
            } 
          })
        ),
        React.createElement("div", { style: { fontSize: 11, textAlign: "right", opacity: 0.7, marginBottom: 12 } }, `${c.progresso.toFixed(1)}% Consumido`),

        React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 6, fontSize: 12, fontWeight: 600, color: c.corPrazo } },
          "📅 ", c.validade ? `${c.validade.split("-").reverse().join("/")} (${c.diasRestantes >= 0 ? `Faltam ${c.diasRestantes} dias` : `Vencido há ${Math.abs(c.diasRestantes)} dias`})` : "Validade não definida"
        )
      )
    )),
    ...
    passwordPrompt && React.createElement("div", {
      ...
      React.createElement("h3", { style: { marginTop: 0, marginBottom: 8, fontSize: 16, fontWeight: 700, color: dark ? "#fff" : "#0f172a" } }, passwordPrompt.title),
      React.createElement("p", { style: { fontSize: 13, opacity: 0.8, marginBottom: 16, color: dark ? "#cbd5e1" : "#475569" } }, "Digite a sua senha de usuário para confirmar esta ação."),
      React.createElement("input", {
        type: "password",
        ...
        onKeyDown: e => {
          if (e.key === "Enter") {
            passwordPrompt.onConfirm(e.target.value);
          }
        },
        style: {
          width: "100%",
      ...
      React.createElement("div", { style: { display: "flex", gap: 10, justifyContent: "flex-end" } },
        ...
        React.createElement("button", {
          onClick: (e) => {
            const inputVal = e.target.parentElement.previousSibling.value;
            passwordPrompt.onConfirm(inputVal);
          },
          style: {
            padding: "8px 16px",
            borderRadius: 6,
